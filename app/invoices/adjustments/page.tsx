...
export default async function AdjustmentNoteRegisterPage({ searchParams }: {
  ...
  return <div className="sf-invoice-page sf-invoice-history-page sf-invoice-register-page">
    <header className="sf-invoice-history-page__header">
      <div><p className="sf-eyebrow">Legal documents</p><h1>Adjustment note register</h1><p>Tenant-scoped immutable Australian GST adjustments linked to independently verified source tax invoices and commercial authority.</p></div>
    ...
    <section className="sf-invoice-history-card" aria-labelledby="adjustment-register-title">
      ...
      {notes.items.length === 0 ? <div className="sf-invoice-history-empty"><h3>No adjustment notes issued</h3><p>Verified Australian adjustment notes will appear here after a supported cancellation or commercial amendment changes the taxable sale represented by an issued tax invoice.</p></div> : <div className="sf-invoice-history-table-wrap"><table className="sf-invoice-history-table sf-invoice-register-table"><thead><tr><th scope="col">Adjustment note</th><th scope="col">Reason</th><th scope="col">Source invoice</th><th scope="col">Booking</th><th scope="col">Issued</th><th scope="col">Effect</th><th scope="col">Document</th></tr></thead><tbody>{notes.items.map((note) => <tr key={note.documentNumber}><th scope="row">{note.documentNumber}</th><td>{note.adjustmentReason}</td><td><Link href={`/invoices/${encodeURIComponent(note.sourceTaxInvoiceNumber)}`}>{note.sourceTaxInvoiceNumber}</Link></td><td><Link href={`/bookings/${encodeURIComponent(note.bookingId)}`}>View booking</Link></td><td>{note.issuedAt.toLocaleDateString('en-AU', { timeZone: 'UTC' })}</td><td>{note.adjustmentType === 'Increasing adjustment' ? `Increase ${money(note.increaseTotalMinor, note.currency)}` : `Decrease ${money(note.decreaseTotalMinor, note.currency)}`}</td><td><Link href={`/invoices/adjustments/${encodeURIComponent(note.documentNumber)}`}>View adjustment note</Link></td></tr>)}</tbody></table></div>}
