...
export async function POST(
  ...
  if (!activeContext.organization) return NextResponse.redirect(new URL('/account?memberError=tenant', request.url), 303);

  let formData: FormData;
  try {
    formData = await request.formData();
  } catch {
    return NextResponse.redirect(new URL('/account?memberError=validation', request.url), 303);
  }
  ...
  try {
    ...
    return NextResponse.redirect(new URL('/account?status=membership-updated', request.url), 303);
  } catch (error) {
    const code = error instanceof OrganizationPermissionDeniedError
      ...
      : error instanceof MembershipStatusConflictError
        ...
        : error instanceof MembershipStatusValidationError
          ? 'validation'
          : 'server';
    return NextResponse.redirect(new URL(`/account?memberError=${code}`, request.url), 303);
  }
}
