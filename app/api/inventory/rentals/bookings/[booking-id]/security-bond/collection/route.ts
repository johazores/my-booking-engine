...
export async function POST(request: Request, { params }: { params: Promise<{ 'booking-id': string }> }) {
  ...
  try {
    ...
    const result = await recordRentalSecurityBondManualCollection({ organizationId: organization.id, actorUserId: session.user.id, bookingId, reference: formData.get('reference') });
