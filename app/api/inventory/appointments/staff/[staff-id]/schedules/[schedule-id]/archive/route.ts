...
import { archiveAppointmentSchedule } from '@/server/inventory/appointment-service.ts';

export async function POST(
  request: Request,
  context: { params: Promise<{ 'staff-id': string; 'schedule-id': string }> },
) {
  const mutation = await prepareInventoryMutationRequest(request, 'inventory.appointment-schedule.archive');
  ...
  try {
    await archiveAppointmentSchedule({
    ...
    return finish(NextResponse.redirect(new URL(`${staffPath}?status=schedule-archived`, request.url), 303));
  } catch (error) {
