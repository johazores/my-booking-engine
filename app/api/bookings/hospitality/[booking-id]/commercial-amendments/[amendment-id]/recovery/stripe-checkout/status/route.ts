...
import { reconcileStripeHospitalityBookingCommercialAmendmentRecoveryTransport } from '@/server/bookings/hospitality-booking-commercial-amendment-recovery-transport-service.ts';
...
export async function POST(request: Request, context: RouteContext) {
  const observation = createRequestObservation(request, { operation: 'payment.commercial-amendment.stripe-recovery-checkout.reconcile' });
  ...
  try {
    ...
    const result = await reconcileStripeHospitalityBookingCommercialAmendmentRecoveryTransport({
      organizationId: auth.organizationId,
