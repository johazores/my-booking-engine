...
const errors: Record<string, string> = {
  ...
  dependency: 'Remove room-type assignments and archive active restrictions before changing this rate plan.',
  ...
  return <div className="sf-inventory-page">
    <header className="sf-inventory-page__header">
      <div>
        ...
        <h1>Rate plans</h1>
        <p>Define sellable commercial plan identities now; prices and availability remain separate later layers.</p>
      </div>
      <div className="sf-image-scope__nav">
        <Link className="sf-button sf-button--secondary sf-button--compact" href={`/inventory/${propertyId}/restrictions`}>Restrictions</Link>
    ...
    <div className={`sf-inventory-layout${canMutateProperty ? '' : ' sf-inventory-layout--single'}`}>
      <section className="sf-inventory-card" aria-labelledby="rate-plans-title"><div className="sf-inventory-card__heading"><div><p className="sf-eyebrow">Commercial definitions</p><h2 id="rate-plans-title">Property rate plans</h2></div><span>{ratePlans.total} total</span></div>
        {ratePlans.ratePlans.length === 0 ? <div className="sf-empty-state"><h3>No rate plans yet</h3><p>{canMutateProperty ? 'Create the first rate plan for this property.' : 'No rate plans are configured for this property.'}</p></div> : <ul className="sf-inventory-list">{ratePlans.ratePlans.map((ratePlan) => <li key={ratePlan.id}><Link className={`sf-inventory-list__link${selectedRatePlan?.id === ratePlan.id ? ' sf-inventory-list__link--selected' : ''}`} href={ratePlanHref(propertyId, { ratePlanId: ratePlan.id, page: ratePlans.page, pageSize })}><div><strong>{ratePlan.name}</strong><span>{ratePlan.code}{ratePlan.description ? ` · ${ratePlan.description}` : ''}</span></div><div className="sf-inventory-list__meta"><span className={`sf-status-badge${ratePlan.status === 'ARCHIVED' ? ' sf-status-badge--muted' : ''}`}>{ratePlan.status.toLowerCase()}</span><span>{ratePlan._count.roomTypeAssignments} room types · {ratePlan._count.restrictions} rules</span></div></Link></li>)}</ul>}
    ...
    {selectedRatePlan && roomTypes ? <section className="sf-inventory-card sf-inventory-rooms" aria-labelledby="rate-plan-room-types-title"><div className="sf-inventory-card__heading"><div><p className="sf-eyebrow">Room-type availability</p><h2 id="rate-plan-room-types-title">{selectedRatePlan.name}</h2><p>Choose which room types can publish prices and restrictions under this plan.</p></div><span>{selectedRatePlan._count.roomTypeAssignments} assigned</span></div>
