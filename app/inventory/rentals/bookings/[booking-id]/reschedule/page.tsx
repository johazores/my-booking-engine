...
const blockerMessages = {
  NO_CHANGE: 'Choose dates that differ from the current rental period.',
...
export default async function RentalBookingRescheduleReviewPage({
  ...
  searchParams: Promise<{ startsOn?: string; endsOn?: string }>;
}) {
  ...
  if (booking.status !== 'CONFIRMED' || !booking.allocation) {
    return <section className="sf-inventory-empty"><p className="sf-eyebrow">Rental reschedule</p><h1>Booking is not eligible</h1><p>Only a confirmed rental booking with its exact retained physical allocation can be reviewed for new dates.</p><Link className="sf-button sf-button--secondary" href={`/inventory/rentals/bookings/${booking.id}`}>Back to booking</Link></section>;
  }
  ...
  const defaultStartsOn = query.startsOn ?? booking.startsOn.toISOString().slice(0, 10);
  const defaultEndsOn = query.endsOn ?? booking.endsOn.toISOString().slice(0, 10);

  return <div className="sf-inventory-page">
    <header className="sf-inventory-page__header">
      <div><p className="sf-eyebrow">Rental booking</p><h1>Reschedule preflight</h1><p>Review target dates for {booking.customerFirstName} {booking.customerLastName} without changing the durable booking.</p></div>
    ...
    <section className="sf-inventory-card" aria-labelledby="rental-reschedule-review-title">
      <div className="sf-inventory-card__heading"><div><p className="sf-eyebrow">Read-only authority</p><h2 id="rental-reschedule-review-title">Target date review</h2></div><span>{booking.unit.name} ({booking.unit.code})</span></div>
      <p>Current period: <strong>{booking.startsOn.toISOString().slice(0, 10)}</strong> through <strong>{booking.endsOn.toISOString().slice(0, 10)}</strong> (end exclusive). This preflight keeps the same physical unit and only returns ready authority when current target pricing has the same currency and exact aggregate total.</p>
      ...
      <p className="sf-field-hint">Reviewing target dates does not reserve new inventory and does not mutate the booking, allocation, payments, deposits, or fulfillment state.</p>
    </section>

    {review ? <section className="sf-inventory-card" aria-labelledby="rental-reschedule-result-title">
      <div className="sf-inventory-card__heading"><div><p className="sf-eyebrow">Authority result</p><h2 id="rental-reschedule-result-title">{review.ready ? 'Ready for a future durable writer' : 'Target dates blocked'}</h2></div><span>{review.targetPricing.currency} {moneyMinorToMajorString(review.targetPricing.totalMinor, review.targetPricing.currency)}</span></div>
      {review.blocker ? <p className="sf-alert sf-alert--error" role="alert">{blockerMessages[review.blocker]}</p> : <p className="sf-alert sf-alert--success" role="status">Inventory and current aggregate pricing are compatible with the narrow price-neutral date-reschedule contract. No booking data changed.</p>}
      ...
      <p className="sf-field-hint">This authority is intentionally read-only. A production reschedule writer must preserve the original accepted snapshot and repeat inventory, pricing, tenant, and stale-authority checks under the booking and physical-unit write locks.</p>
    </section> : null}
  </div>;
}
