...
const errors: Record<string, string> = {
  ...
  validation: 'The security bond amount or external reference was invalid.',
...
export default async function RentalSecurityBondPage({ params, searchParams }: {
  ...
  try {
    data = await readRentalSecurityBond({ organizationId: activeContext.organization.id, actorUserId: session.user.id, bookingId });
  } catch (error) {
  ...
  return <div className="sf-inventory-page">
    <header className="sf-inventory-page__header">
      <div><p className="sf-eyebrow">Rental security bond</p><h1>{data.booking.customerFirstName} {data.booking.customerLastName}</h1><p>Retained manual/offline bond requirement, collection, and release evidence.</p></div>
    ...
    <section className="sf-inventory-card" aria-labelledby="rental-security-bond-title">
      <div className="sf-inventory-card__heading"><div><p className="sf-eyebrow">Commercial custody safeguard</p><h2 id="rental-security-bond-title">Security bond</h2></div><span>{data.settlement?.state ?? 'NOT REQUIRED'}</span></div>
      {data.bond && data.settlement ? <>
