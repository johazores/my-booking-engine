...
function parseCustody(value: string | undefined): RentalBookingListCustody {
  return value === 'overdue' ? 'OVERDUE' : 'ALL';
}
...
function bookingListHref(input: Readonly<{
  ...
  if (input.custody !== 'ALL') params.set('custody', input.custody.toLowerCase());
...
export default async function RentalBookingsPage({
  ...
  return <div className="sf-inventory-page">
    ...
    <section className="sf-inventory-card" aria-labelledby="rental-booking-filter-title">
      <div className="sf-inventory-card__heading"><div><p className="sf-eyebrow">History</p><h2 id="rental-booking-filter-title">Booking records</h2></div><span>{result.total} matching · {result.overdueCount} overdue custody</span></div>
      <form method="get" className="sf-form-row">
        ...
        <label className="sf-field">Custody<select name="custody" defaultValue={custody.toLowerCase()}><option value="all">All</option><option value="overdue">Overdue only</option></select></label>
      ...
      {result.bookings.length === 0 ? <div className="sf-empty-state"><h3>{custody === 'OVERDUE' ? 'No overdue rental custody matches this filter' : 'No rental bookings match this filter'}</h3><p>{custody === 'OVERDUE' ? 'Overdue custody applies only to confirmed picked-up bookings whose exclusive expected-return boundary has been reached without a recorded return.' : 'Bookings appear only after an effective hold and active customer pass the server-side conversion review and atomic confirmation boundary.'}</p>{custody === 'OVERDUE' ? <Link className="sf-button sf-button--secondary" href={bookingListHref({ page: 1, pageSize, status: 'ALL', custody: 'ALL' })}>View all rental bookings</Link> : <Link className="sf-button sf-button--primary" href="/inventory/rentals/holds">Review active holds</Link>}</div> : <ul className="sf-inventory-list">{result.bookings.map((booking) => {
