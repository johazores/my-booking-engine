...
export default async function RentalBookingsPage({
  ...
  return <div className="sf-inventory-page">
    ...
    <section className="sf-inventory-card" aria-labelledby="rental-booking-filter-title">
      ...
      <p className="sf-field-hint">A confirmed rental booking means SF has committed the current effective physical unit under reviewed commercial evidence. The committed period follows supported reschedules; live inventory protection may end earlier only after an explicit post-return release. Original booking-time evidence remains retained.</p>

      {result.bookings.length === 0 ? <div className="sf-empty-state"><h3>No rental bookings match this filter</h3><p>Bookings appear only after an effective hold and active customer pass the server-side conversion review and atomic confirmation boundary.</p><Link className="sf-button sf-button--primary" href="/inventory/rentals/holds">Review active holds</Link></div> : <ul className="sf-inventory-list">{result.bookings.map((booking) => {
