...
export default async function RentalUnitPage({
  ...
  return <div className="sf-inventory-page">
    <header className="sf-inventory-page__header"><div><p className="sf-eyebrow">Rental unit</p><h1>{inventory.unit.name}</h1><p>{inventory.unit.code} · {inventory.unit.unitType.name} · {inventory.unit.location ? `${inventory.unit.location.name} (${inventory.unit.location.code})` : 'Location not assigned'} · {operationalState.status === 'OUT_OF_SERVICE' ? 'Out of service' : 'Available'}</p></div><div className="sf-image-scope__nav"><Link className="sf-button sf-button--secondary" href={`/inventory/rentals/types/${inventory.unit.unitTypeId}`}>Unit type</Link>{inventory.unit.location ? <Link className="sf-button sf-button--secondary" href={`/inventory/rentals/locations/${inventory.unit.location.id}`}>Location</Link> : null}<Link className="sf-button sf-button--secondary" href="/inventory/rentals">All rentals</Link></div></header>
