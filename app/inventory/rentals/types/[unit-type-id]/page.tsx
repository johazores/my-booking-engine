  ...
  return <div className="sf-inventory-page">
    ...
    <div className={`sf-inventory-layout${canManage ? '' : ' sf-inventory-layout--single'}`}>
      <section className="sf-inventory-card"><div className="sf-inventory-card__heading"><div><p className="sf-eyebrow">Units</p><h2>Physical rental units</h2></div><span>{inventory.units.total} active</span></div>
        {inventory.units.items.length === 0 ? <div className="sf-empty-state"><h3>No units yet</h3><p>Add the first physical unit for this type.</p></div> :
          <ul className="sf-inventory-list">{inventory.units.items.map((unit) => <li key={unit.id}><div className="sf-inventory-list__link"><Link className="sf-inventory-list__primary" href={`/inventory/rentals/units/${unit.id}`}><div><strong>{unit.name}</strong><span>{unit.code}</span></div></Link></div></li>)}</ul>}
      ...
      {canManage ? <aside className="sf-inventory-card sf-inventory-card--create"><p className="sf-eyebrow">New unit</p><h2>Add physical unit</h2><form className="sf-form" action="/api/inventory/rentals/units" method="post"><input type="hidden" name="unitTypeId" value={inventory.unitType.id} /><label className="sf-field">Name<input name="name" maxLength={160} required /></label><label className="sf-field">Code<input name="code" maxLength={32} required /></label><label className="sf-field">Description<textarea name="description" maxLength={1000} rows={3} /></label><button className="sf-button sf-button--primary" type="submit">Add unit</button></form></aside> : null}
    </div>
