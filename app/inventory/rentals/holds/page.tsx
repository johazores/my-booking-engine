...
export default async function RentalAvailabilityHoldsPage({
  ...
  return <div className="sf-inventory-page">
    <header className="sf-inventory-page__header">
      <div><p className="sf-eyebrow">Rental availability</p><h1>Active temporary holds</h1><p>Review the short-lived physical-unit holds currently protecting rental inventory for {activeContext.organization.name}.</p></div>
    ...
    <section className="sf-inventory-card" aria-labelledby="rental-holds-title">
      ...
      <p className="sf-field-hint">Expired holds stop protecting inventory automatically even before lifecycle cleanup. These records are inventory protection only and are not customer reservations or payment authority.</p>
      {holds.items.length === 0 ? <div className="sf-empty-state"><h3>No effective rental holds</h3><p>Use the availability preview to place a short-lived hold on an available physical unit when operational review needs temporary inventory protection.</p></div> : <ul className="sf-inventory-list">{holds.items.map((hold) => <li key={hold.id}><div className="sf-inventory-list__link"><div className="sf-inventory-list__primary"><div><strong>{hold.unit.name}</strong><span>{hold.unit.code} · {hold.unit.unitType.name} ({hold.unit.unitType.code}) · {hold.unit.location ? `${hold.unit.location.name} (${hold.unit.location.code})` : 'Location unavailable'}</span><span>{hold.startsOn.toISOString().slice(0, 10)} through {hold.endsOn.toISOString().slice(0, 10)} (end exclusive) · expires <time dateTime={hold.expiresAt.toISOString()}>{hold.expiresAt.toISOString()}</time></span></div></div>{canManage ? <form action={`/api/inventory/rentals/holds/${hold.id}/release`} method="post"><button className="sf-button sf-button--secondary sf-button--compact" type="submit">Release hold</button></form> : null}</div></li>)}</ul>}
