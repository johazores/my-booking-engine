...
export default async function InventoryPage({ searchParams }: { searchParams: Promise<{ page?: string; pageSize?: string; status?: string; error?: string }> }) {
  ...
  return <div className="sf-inventory-page">
    <header className="sf-inventory-page__header"><div><p className="sf-eyebrow">Hospitality inventory</p><h1>Properties</h1><p>Manage the real property → room type → room hierarchy for {activeContext.organization.name}.</p></div><div className="sf-image-scope__nav"><span className="sf-inventory-count">{result.total} properties</span><Link className="sf-button sf-button--secondary" href="/inventory/tours">Tours and packages</Link><Link className="sf-button sf-button--secondary" href="/inventory/appointments">Appointments</Link><Link className="sf-button sf-button--secondary" href="/inventory/rentals">Rentals</Link></div></header>
    ...
    <div className={`sf-inventory-layout${canManage ? '' : ' sf-inventory-layout--single'}`}>
      <section className="sf-inventory-card" aria-labelledby="properties-title">
        ...
        {result.properties.length === 0 ? <div className="sf-empty-state"><h3>No properties yet</h3><p>{canManage ? 'Create the first property to establish hospitality inventory.' : 'No properties are available for this tenant.'}</p></div> : <ul className="sf-inventory-list">{result.properties.map((property) => <li key={property.id}><div className="sf-inventory-list__link"><Link className="sf-inventory-list__primary" href={`/inventory/${property.id}`}><div><strong>{property.name}</strong><span>{property.code} · {property.city ?? property.countryCode}</span></div></Link><div className="sf-inventory-list__meta"><span className={`sf-status-badge${property.status === 'ARCHIVED' ? ' sf-status-badge--muted' : ''}`}>{property.status.toLowerCase()}</span><span>{property._count.roomTypes} room types</span><div className="sf-image-scope__nav"><Link className="sf-button sf-button--secondary sf-button--compact" href={`/inventory/${property.id}/rate-plans`}>Rate plans</Link><Link className="sf-button sf-button--secondary sf-button--compact" href={`/inventory/${property.id}/images`}>Images</Link></div></div></div></li>)}</ul>}
