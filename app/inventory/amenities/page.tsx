...
import { listHospitalityAmenitiesPage } from '@/server/inventory/hospitality-amenity-service.ts';
...
function amenityPageHref(page: number, pageSize: number) {
  const params = new URLSearchParams();
  if (page > 1) params.set('page', String(page));
  if (pageSize !== 20) params.set('pageSize', String(pageSize));
  const query = params.toString();
  return query ? `/inventory/amenities?${query}` : '/inventory/amenities';
}
...
export default async function AmenitiesPage({
  ...
  const result = await listHospitalityAmenitiesPage({
  ...
  return <div className="sf-inventory-page">
    <header className="sf-inventory-page__header"><div><Link className="sf-back-link" href="/inventory">← Inventory</Link><p className="sf-eyebrow">Hospitality inventory</p><h1>Amenities</h1><p>Maintain reusable tenant-owned amenities and assign them to properties or room types.</p></div><span className="sf-inventory-count">{result.total} amenities</span></header>
    ...
    <div className={`sf-inventory-layout${canManage ? '' : ' sf-inventory-layout--single'}`}>
      <section className="sf-inventory-card" aria-labelledby="amenities-title">
        <div className="sf-inventory-card__heading"><div><p className="sf-eyebrow">Reusable definitions</p><h2 id="amenities-title">Amenity records</h2></div><span>{first}–{last} of {result.total}</span></div>
        {result.amenities.length === 0 ? <div className="sf-empty-state"><h3>No amenities yet</h3><p>{canManage ? 'Create reusable amenities before assigning them to properties or room types.' : 'No amenities are configured for this tenant.'}</p></div> : <div className="sf-room-table-wrap"><table className="sf-room-table"><thead><tr><th scope="col">Amenity</th><th scope="col">Code</th><th scope="col">Assignments</th><th scope="col">Status</th>{canManage ? <th scope="col">Action</th> : null}</tr></thead><tbody>{result.amenities.map((amenity) => <tr key={amenity.id}><th scope="row">{amenity.name}</th><td>{amenity.code}</td><td>{amenity._count.propertyAssignments} properties · {amenity._count.roomTypeAssignments} room types</td><td><span className={`sf-status-badge${amenity.status === 'ARCHIVED' ? ' sf-status-badge--muted' : ''}`}>{amenity.status.toLowerCase()}</span></td>{canManage ? <td>{amenity.status === 'ACTIVE' ? <form action={`/api/inventory/amenities/${amenity.id}/archive`} method="post" className="sf-archive-inline"><input className="sf-archive-inline__input" name="confirmation" aria-label={`Type ARCHIVE to archive amenity ${amenity.name}`} placeholder="ARCHIVE" required /><button className="sf-button sf-button--danger sf-button--compact" type="submit">Archive</button></form> : '—'}</td> : null}</tr>)}</tbody></table></div>}
        {result.totalPages > 1 ? <nav className="sf-pagination" aria-label="Amenity pages">{result.page > 1 ? <Link className="sf-button sf-button--secondary sf-button--compact" href={amenityPageHref(result.page - 1, result.pageSize)}>Previous</Link> : <span />}<span>Page {result.page} of {result.totalPages}</span>{result.page < result.totalPages ? <Link className="sf-button sf-button--secondary sf-button--compact" href={amenityPageHref(result.page + 1, result.pageSize)}>Next</Link> : <span />}</nav> : null}
      </section>
