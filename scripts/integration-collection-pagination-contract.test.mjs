...
test('integration collection service bounds complete and paginated reads', () => {
  ...
  assert.match(service, /const total = await db\.integration\.count\(\{ where \}\)/);
...
test('integration reads preserve tenant authorization and exact featured-provider identity', () => {
  ...
  assert.match(service, /readIntegrationHealthEvent/);
...
test('documentation records the collection and credential-safety boundaries', () => {
