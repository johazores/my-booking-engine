...
test('application writers serialize fresh child authority with parent archival', async () => {
  ...
  assert.match(service, /assignRentalUnitLocation[\s\S]*lockRentalUnit\(transaction, input\.organizationId, input\.unitId\)[\s\S]*lockRentalLocationLifecycle\(transaction, input\.organizationId, locationLocator\.id\)[\s\S]*status: 'ACTIVE'[\s\S]*data: \{ locationId: location\.id \}/);
