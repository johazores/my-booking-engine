...
const services = [confirmation, cancellation, reschedule, substitution];
...
test('all rental booking writers use the shared classifier instead of local Prisma parsing', () => {
  ...
  assert.match(substitution, /retryUniqueConflict: true/);
  assert.doesNotMatch(cancellation, /retryUniqueConflict: true/);
});

test('expected persistence failures remain explicit booking conflicts at all staff routes', () => {
...
test('documentation keeps retries bounded and unknown failures visible', () => {
  assert.match(docs, /four durable rental booking write paths/);
