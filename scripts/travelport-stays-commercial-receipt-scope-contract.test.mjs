...
test('commercial Travelport receipt authority is bound to offer scope', () => {
  ...
  assert.match(classifier, /!offerEvidence\.matchedOfferId \|\| offerRef !== offerEvidence\.matchedOfferId/);
  assert.match(classifier, /!commercialReceiptScopeIsValid\(reservation\.Receipt, offerEvidence\)/);
});

test('commercial receipt scope remains provider-specific and does not enable reservation capability', () => {
