...
test('rental booking read model requires booking read permission and repeats tenant scope', () => {
  ...
  assert.match(readService, /allocation: \{ include: effectiveAllocationInclude \}/);
...
test('staff booking lifecycle exposes only supported durable mutations', () => {
  assert.match(bookingList, /listRentalBookings/);
  assert.match(bookingList, /booking\.allocation\.unit\.name/);
  assert.match(bookingList, /unit-substitution/);
  assert.match(bookingDetail, /unit-substitution/);
  assert.match(bookingDetail, /Physical-unit substitution evidence/);
  ...
  for (const unsupportedAction of ['Collect payment', 'Take deposit', 'Change unit type', 'Change location', 'Complete pickup', 'Complete return']) {
...
test('documentation preserves the production boundary and forbids fake downstream workflow claims', () => {
  ...
  assert.match(foundation, /Physical-unit substitution lifecycle/);
  assert.match(inventoryDoc, /Staff conversion interaction/);
  assert.match(inventoryDoc, /same-type, same-location physical-unit substitution/i);
});
