...
SF rental inventory is a tenant-owned production foundation for rentable physical inventory. It separates catalog, operating-location, availability-calendar, temporary inventory protection, pricing evidence, rate configuration, durable physical-unit allocation, staff rental booking lifecycle, physical-custody evidence, operational availability, maintenance, post-return condition/damage evidence, security-bond evidence, late-return commercial evidence, and the current narrow manual settlement boundaries. The current booking workflow supports hold-to-booking confirmation, tenant-scoped history/detail, same-unit price-neutral date rescheduling, same-unit price-neutral custody extension after pickup, same-type/same-location physical-unit substitution before pickup, terminal pre-pickup inventory-release cancellation, staff-only partial/full manual/offline booking-price settlement across multiple retained sources with source-attributed partial/full refunds, append-only pickup/return custody evidence, overdue open-custody inventory protection, missed-pickup operational visibility, explicit release of complete remaining rental days after return, operational out-of-service/maintenance work orders, return inspection and damage-case follow-up, customer damage-liability authority with exact manual settlement or exact security-bond forfeiture, versioned unit-type late-return fee policy revisions with non-retroactive automatic fee math, and explicit late-return assessment with exact manual/offline fee settlement. Online/card or other provider-backed rental settlement, unit-type/location-changing and price-changing amendments/extensions, delivery semantics, notifications/external synchronization, and customer self-service remain separate contracts.
...
- Pickup freezes cancellation, physical-unit substitution, and arbitrary date moves. While custody is open, the separate reschedule lifecycle permits only a same-unit, current-start, strictly later-end extension that preserves accepted currency and exact aggregate price; return closes that exception.
...
- Database-enforced tenant roots, organization-composite relationships, booking insert guards, immutable booking-time evidence, append-only reschedule/substitution/payment/fulfillment/early-return-release evidence, effective-allocation guards, terminal cancellation and financial-settlement guards, custody-order, pickup-window, missed-pickup substitution, custody-extension, and overdue-custody guards, per-unit advisory serialization, and deferred exact-allocation validation.
...
- Audit events for inventory, hold, booking confirmation, booking reschedule/extension, booking unit substitution, booking cancellation, rental manual payment/refund recording, pickup, return, and early-return inventory release.
...
- Real inventory UI under `/inventory/rentals` with unit-type, location, unit, availability, hold, conversion, booking list/detail, reschedule/extension, unit-substitution, cancellation, manual settlement, pickup, return, and post-return inventory-release surfaces.
...
### Same-unit price-neutral rescheduling and custody extension

Authorized staff may review a new date range for the current effective confirmed physical unit before pickup. After pickup and before return, the same lifecycle becomes a narrow custody-extension contract: the effective start and physical unit are fixed, the target end must move strictly later, and current target pricing must still preserve the accepted currency and exact aggregate amount. Review requires `booking:manage`, `availability:read`, `inventory:read`, and `pricing:read`; apply additionally requires `availability:manage`.
...
Apply reacquires tenant/booking and effective physical-unit locks, rebuilds custody-aware authority, derives idempotency server-side, inserts append-only `rental_booking_reschedules` evidence, moves only effective allocation dates, advances booking version, and records audit evidence. The authority fingerprint binds whether the decision is pre-pickup or in-custody and, after pickup, the immutable pickup-event ID. Database guards reject returned bookings and independently constrain an in-custody insert to the current-start/later-end shape while cancellation and unit replacement remain closed after pickup.
...
A confirmed rental booking may receive multiple real manual/offline payments recorded by authorized staff after each external settlement has occurred. Staff provide the normalized external reference and a positive major-unit amount up to the current outstanding booking balance; tenant, actor, booking, idempotency, currency, authoritative outstanding balance, and exact minor-unit conversion are server-derived. Each external reference is isolated tenant-wide across booking-price, damage, security-bond, and late-return commercial ledgers before provider-adapter I/O, with PostgreSQL enforcing the same cross-ledger boundary independently.
...
Payment evidence is append-only and separate from booking/inventory/fulfillment evidence. This booking-price workflow does not implement card authorization, Stripe rental checkout, online split-tender checkout, mixed-provider settlement, cancellation fees, chargebacks, customer self-service, or pickup/return-specific settlement rules. Security bonds, customer-damage settlement, and late-return settlement use their own retained evidence streams. See [rental-payment-foundation.md](./rental-payment-foundation.md).
...
Pickup is the physical-custody handoff boundary. From that point, database guards reject cancellation, physical-unit substitution, and arbitrary date changes so historical custody evidence cannot be contradicted. A separate custody-extension guard permits only a same-unit, current-start, strictly later-end price-neutral extension before return; it binds the immutable pickup event and cannot reopen broader rescheduling authority.
...
Price-changing or broader rental extensions/amendments, delivery semantics, provider-backed or mixed-provider rental settlement, customer self-service, fulfillment notifications, and external synchronization remain separate contracts.
...
- online rental checkout/card authorization, online split-tender or mixed-provider/provider-backed booking-price settlement, provider-backed damage/bond/late-fee settlement, cancellation fees, discounts, taxes/fees beyond retained explicit authorities, or complex pricing beyond current daily-rate evidence
- customer pickup/drop-off location selection, one-way return policy, delivery zones, opening-hour rules, or transfer pricing
- price-changing or broader rental extensions/amendments, delivery semantics, customer self-service, or fulfillment notifications
