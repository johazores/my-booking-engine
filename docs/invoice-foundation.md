...
Fiscal numbering is independently protected at the persistence boundary. `HospitalityInvoiceNumberSequence` keys are immutable, counters may move by one only, and deferred integrity checks require each supported tenant/jurisdiction/document ledger to start at `1`, remain contiguous, retain its sequence row, and leave `nextValue` exactly one greater than issued history. Counter advancement without its matching immutable document, document insertion without matching counter state, and sequence deletion while issued history remains all fail at transaction commit. The detailed invariant is in `docs/invoice-number-sequence-integrity.md`.
