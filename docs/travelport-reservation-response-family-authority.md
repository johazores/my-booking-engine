...
Travelport reservation responses can affect irreversible Create settlement, commercial review, Booking.com Sync recovery, and known-locator reconciliation. The top-level response family, HTTP status, and structured response media type are therefore machine authority, not compatibility hints.
...
After the exact v11 response-header trace has been bound to the durable SF attempt, the reservation transport requires a structured response to declare `application/json` before SF reads or parses its body. An optional charset parameter is accepted only when it is UTF-8. Missing media type, non-JSON media types, structured-suffix alternatives such as `application/problem+json`, non-UTF-8 charset declarations, and unsupported extra media-type parameters fail closed as `INVALID_RESPONSE`.

After JSON parsing and exact payload-trace binding, the reservation transport requires exactly one supported payload family:
...
For Create and Booking.com Sync, a response media-type/family/status contradiction after the durable provider-request marker cannot prove that no external sell occurred. It therefore fails closed through the existing post-provider ambiguity handling rather than becoming automatic retry authority.

Known-locator Retrieve is read-only. A media-type/family/status contradiction remains `INVALID_RESPONSE`; it cannot prove `FOUND`, `NOT_FOUND`, or supplier lifecycle state.
