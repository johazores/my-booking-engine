...
SF has an authenticated, tenant-scoped price-impact review contract, a durable amendment boundary for non-zero commercial booking changes, amendment-attributed settlement/reconciliation, a provider-neutral execution-decision contract, real manual adjustment recording, source-scoped Stripe amendment refunds with reconciliation, customer-authorized Stripe Checkout for normal price increases, provider-known signed amendment webhook finalization, explicit expired-amendment recovery, recovery-owned signed callback convergence, post-settlement apply-failure recovery routing, and a final serializable apply boundary. The booking workspace orchestrates reviewed non-zero changes through preparation, safe provider-specific settlement states, reconciliation, cancellation, final apply, and compensation recovery when settled money cannot be applied safely.
...
`prepareHospitalityBookingCommercialAmendment` remains the authoritative preparation service. The authenticated commercial-amendment transport exposes it only after a server price review, and accepts the reviewed adjustment fingerprint rather than browser-supplied money or settlement authority.
...
The normal booking workspace now uses a separate customer-authorized Stripe-hosted Checkout boundary for price increases. `STRIPE_CUSTOMER_AUTHORIZATION_REQUIRED` creates or resumes one deterministic amendment-owned Checkout attempt with exact server-derived money, tenant/booking/amendment metadata, and server return URLs. The customer enters payment details only on Stripe; browser return navigation is never payment truth. Polling and signed Checkout callbacks reconcile the exact Session/PaymentIntent into amendment-owned evidence before final apply.

Signed PaymentIntent/refund callbacks can finalize provider-known normal amendment `AMBIGUOUS` evidence. The public Stripe route first runs tenant-secret signature verification and durable event ingestion. `finalizeVerifiedStripeCommercialAmendmentWebhook` then requires the exact verified event ID/type/hash and persisted provider reference, tenant/booking/amendment identity, currency, amount, and refund source/fingerprint where applicable. The callback updates only amendment payment evidence plus `PaymentWebhookEvent`; it never rewrites booking payment/commercial state. It intentionally refuses to guess ownership for a row that still has only an internal `sf_claim_*` reference.
...
When target protection is required, apply also requires the exact active hold created for this amendment: tenant, hold ID, deterministic hold idempotency identity, property, room type, rate plan, stay dates, protected quantity, and expiry must all still match. An expired or mismatched hold fails closed.
...
If that final apply transaction fails with a known booking conflict, price change, or booking-unavailable condition after the exact adjustment money is already settled, the transport does not leave real money inside an otherwise live prepared amendment. After the failed apply transaction rolls back, `routeSettledHospitalityBookingCommercialAmendmentApplyFailureToRecovery` independently re-reads the tenant-owned amendment and complete payment ledger under the booking advisory lock in a new serializable transaction. Only a fresh `READY_TO_APPLY` settlement proof can shorten the amendment expiry, release remaining target protection, and write recovery-required audit evidence. Pending, ambiguous, incomplete, or conflicting settlement never gains compensation authority from an apply error.

The amendment remains `PREPARED` but is now expired, so the existing mutation guards and expired-amendment recovery domain own the settled money immediately. Booking commercial/payment state remains untouched. Recovery can compensate automatically where its provider contract is safe, require customer-authorized compensation Checkout where needed, or fail closed for operator reconciliation if the original booking snapshot itself drifted.
...
The Phase 13 general commercial modification item remains open because production completion requires more than connected code paths. Reviewed preparation, manual/Stripe settlement, normal customer-authorized Stripe Checkout, guarded cancellation, final serializable apply, explicit expiry recovery, recovery Checkout, signed/polling convergence, and post-settlement apply-failure recovery routing are now implemented boundaries.

The remaining authority is GitHub issue #1 plus environment-backed verification. Full Node 24 repository validation, Prisma/migration validation, PostgreSQL concurrency tests, real-provider operational checks, and any still-open invoice/tax or release requirements must pass before Phase 13 is marked complete. No flow should be reopened or checklist item marked done merely because dependency-free domain tests pass.

The current 15-minute protection window is intentionally not extended merely because settlement started. Amendment-attributed payment evidence pins an expired amendment into recovery: competing booking/payment mutations, automatic expiry, and explicit cancellation fail closed until reconciliation proves definitive failure or compensation resolves the money state. Normal and recovery Stripe Checkout remain separate authorities with different lifecycle purposes.

## Validation

Dependency-free amendment/payment-domain tests cover room-type changes, same-room quantity changes, bounded expiry, deterministic hold identity, malformed fingerprints, refund-to-source attribution, source balances, legacy single-source inference, legacy multi-source fail-closed behavior, source-level over-refunds, deterministic next-source allocation, source-scoped execution planning, input-order independence, stable tie-breaking, exact bigint money, net-settlement refund availability, sequential manual/Stripe multi-source refunds, amendment-attributed additional settlement/refund progression, authorization-versus-capture semantics, partial source-split adjustment refunds, settlement drift, over-settlement, unresolved-operation conflicts, provider-neutral amendment execution decisions, expired-before-money versus recovery-required lifecycle decisions, final-apply consistency checks, Stripe amendment-refund and amendment-charge exactness, signed amendment/recovery webhook identity, Checkout reconciliation, transport-state mapping, and post-settlement apply-failure recovery routing.

Focused apply-failure routing coverage verifies settled/unexpired routing, already-expired non-extension, unresolved settlement preservation, ledger-conflict preservation, and terminal-state rejection. The modified service and transport also pass the available dependency-free TypeScript syntax parser.

Full repository typecheck/lint/test/build, Prisma validation/migration execution, and PostgreSQL integration tests still require the repository Node 24 toolchain and a confirmed disposable PostgreSQL target. Manual/Stripe amendment executors, product transport routes, normal/recovery signed callback finalizers, verified-event promotion/locking, polling-webhook concurrency, target-hold release, post-settlement recovery routing, recovery terminalization, and final apply therefore remain subject to that database-backed validation gate before Phase 13 can be considered complete.
