...
SF has an authenticated, tenant-scoped price-impact review contract, a durable internal amendment boundary for non-zero commercial booking changes, amendment-attributed settlement/reconciliation, a provider-neutral execution-decision contract, internal manual adjustment execution, amendment-owned Stripe refund and additional-charge execution/reconciliation boundaries, provider-known signed amendment webhook finalization, explicit expired-amendment recovery, recovery-owned signed callback convergence, and an internal final serializable apply boundary. Customer-facing Stripe collection/authentication transport and user-facing settlement orchestration remain deliberately closed until fresh customer-authorized compensation charging and the remaining production validation gates are complete.
...
## Internal Stripe refund execution and recovery

`refundStripeHospitalityBookingCommercialAmendment` is an internal amendment-owned money-moving boundary for the refund direction only. It requires `booking:manage` and `payment:manage`, resolves the active Stripe integration server-side, serializes tenant idempotency plus booking mutation/payment locks, revalidates the prepared booking version and original money, and derives the exact refund source and amount from the complete payment ledger and amendment execution decision. The caller cannot provide a PaymentIntent, amount, currency, or settlement source.
...
Larger amendment refunds can continue source by source with a fresh idempotency key after each definitive successful refund. Once amendment settlement reaches `READY_TO_APPLY`, the existing final serializable apply service remains the only boundary allowed to rewrite the booking snapshot.
...
The customer-facing collection transport is still intentionally separate. A production browser flow must obtain the Stripe PaymentMethod through a Stripe-hosted/Stripe.js boundary and handle required customer authentication safely; this internal executor is not itself an exposed payment form or browser authority. Detailed charge semantics are documented in `docs/stripe-commercial-amendment-charges.md`.
...
Preparing an amendment still does not charge, refund, apply inventory changes, or rewrite booking/payment state. The internal manual settlement executor can record a real manual adjustment, the Stripe refund executor can move/reconcile amendment-owned refund money, and the Stripe additional-charge executor can authorize/capture/reconcile amendment-owned charge money when supplied a real Stripe-issued PaymentMethod. Provider-known ambiguous normal and recovery Stripe operations can finalize from signed callback evidence or dedicated polling. None of these internal boundaries is exposed as an amendment route or primary UI action. The internal final apply service is also not exposed. The Phase 13 general commercial modification item remains open.

Explicit expired-hold/payment recovery now exists for manual compensation and for Stripe authorization release, compensation capture, adjustment-source compensation refund, polling reconciliation, and signed provider-known recovery callbacks. The remaining online recovery dependency is a fresh customer-authorized Stripe compensation charge when an expired refund amendment has reduced net settlement below the original booking total. Existing generic `PENDING` finalizers intentionally do not claim amendment ownership, and signed amendment callbacks deliberately do not bind internal pre-reference `sf_claim_*` rows without exact local provider identity; those remain exact-retry/polling/operator recovery cases.

The current 15-minute protection window is intentionally not extended merely because settlement started. Amendment-attributed payment evidence pins an expired amendment into recovery: competing booking/payment mutations, automatic expiry, and explicit cancellation fail closed until reconciliation proves definitive failure or compensation resolves the money state. No user-facing settlement/apply action should be opened until fresh customer payment authority, recovery completion, and the required database-backed concurrency/tenant/inventory validation are coherent end to end.

## Validation

Dependency-free amendment/payment-domain tests cover room-type changes, same-room quantity changes, bounded expiry, deterministic hold identity, malformed fingerprints, refund-to-source attribution, source balances, legacy single-source inference, legacy multi-source fail-closed behavior, source-level over-refunds, deterministic next-source allocation, source-scoped execution planning, input-order independence, stable tie-breaking, exact bigint money, net-settlement refund availability, sequential manual/Stripe multi-source refunds, amendment-attributed additional settlement/refund progression, authorization-versus-capture semantics, partial source-split adjustment refunds, settlement drift, over-settlement, unresolved-operation conflicts, provider-neutral amendment execution decisions, expired-before-money versus recovery-required lifecycle decisions, final-apply consistency checks for booking version/current terms/current price, target selection, inventory protection, target price, adjustment identity, expiry-guard behavior, Stripe amendment-refund claim/reconciliation exactness, Stripe amendment-charge stage identity/reconciliation exactness, exact signed amendment-webhook provider-reference/source matching, and deterministic expired-recovery callback identity/source enforcement.

The focused Stripe amendment-charge domain suite passes 9/9 under the available runtime. The focused normal amendment-webhook candidate suite passes 8/8. The focused recovery-webhook identity suite passes 4/4 and covers deterministic operation key/fingerprint validation, exact capture source identity, exact refund settlement-source binding, and missing-source rejection. The new recovery webhook domain/service/route files pass the available TypeScript syntax parser. The Stripe amendment refund and recovery services remain covered by their focused dependency-free domain suites as well.

Full repository typecheck/lint/build, Prisma validation/migration execution, and PostgreSQL integration tests still require the repository Node 24 toolchain and a confirmed disposable PostgreSQL target. The manual/Stripe amendment executors, normal/recovery signed callback finalizers, verified-event promotion/locking, polling-webhook concurrency, recovery terminalization, and final apply service therefore remain subject to that database-backed validation gate before Phase 13 can be considered complete.
