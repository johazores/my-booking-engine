...
SF has an authenticated, tenant-scoped price-impact review contract, a durable internal amendment boundary for non-zero commercial booking changes, amendment-attributed settlement/reconciliation, a provider-neutral execution-decision contract, internal manual adjustment execution, an amendment-owned Stripe refund execution/reconciliation boundary, and an internal final serializable apply boundary. Stripe additional-charge collection, signed amendment-webhook finalization, and user-facing settlement orchestration remain deliberately closed until retry, ambiguity, expiry, recovery, and compensation semantics are complete.
...
Stripe additional-charge amendments remain closed because SF does not reuse a prior settlement credential as authority for a new charge. That direction needs a real customer payment-collection transport and its own amendment-aware authorization/capture recovery contract.
...
Preparing an amendment still does not charge, refund, apply inventory changes, or rewrite booking/payment state. The internal manual settlement executor can record a real manual adjustment that already happened outside SF, and the internal Stripe refund executor can move/reconcile amendment-owned refund money, but neither is exposed as a route or primary UI action. The internal final apply service is also not exposed. The Phase 13 general commercial modification item remains open.

The next online dependency is Stripe additional-charge collection and amendment-aware authorization/capture recovery. Signed Stripe webhook finalization for amendment-owned operations also remains open; the refund executor deliberately uses `AMBIGUOUS` claims so existing generic `PENDING` finalizers cannot claim amendment ownership while dedicated exact-retry and polling recovery remain available. After provider execution is complete, SF still needs explicit expired-hold compensation/recovery before any user-facing settlement/apply orchestration can be opened safely.
