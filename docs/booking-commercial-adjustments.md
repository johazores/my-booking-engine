...
SF has an authenticated, tenant-scoped price-impact review contract, a durable internal amendment boundary for non-zero commercial booking changes, amendment-attributed settlement/reconciliation, a provider-neutral execution-decision contract, internal manual adjustment execution, amendment-owned Stripe refund and additional-charge execution/reconciliation boundaries, and an internal final serializable apply boundary. Customer-facing Stripe additional-charge collection transport, signed amendment-webhook finalization, and user-facing settlement orchestration remain deliberately closed until authentication, retry, ambiguity, expiry, recovery, and compensation semantics are complete.
...
Preparing an amendment still does not charge, refund, apply inventory changes, or rewrite booking/payment state. The internal manual settlement executor can record a real manual adjustment, the Stripe refund executor can move/reconcile amendment-owned refund money, and the Stripe additional-charge executor can authorize/capture/reconcile amendment-owned charge money when supplied a real Stripe-issued PaymentMethod. None of these internal boundaries is exposed as a route or primary UI action. The internal final apply service is also not exposed. The Phase 13 general commercial modification item remains open.

The next online dependencies are a customer-facing Stripe collection/authentication transport and signed Stripe webhook finalization for amendment-owned `AMBIGUOUS` authorization/capture/refund rows. Existing generic `PENDING` finalizers intentionally do not claim amendment ownership. After those provider paths are complete, SF still needs explicit expired-hold compensation/recovery before any user-facing settlement/apply orchestration can be opened safely.
