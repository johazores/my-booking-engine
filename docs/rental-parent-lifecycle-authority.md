...
Rental locations and rental unit types are parent inventory authority for active physical units. Unit types also own future rate-period configuration and append-only late-return fee policy revisions. A composite foreign key proves tenant ownership, but it does not prove that the retained parent is still `ACTIVE` while a concurrent child write is being committed.

SF therefore serializes parent archival with every fresh child write that depends on an active parent. This closes the race where a unit, rate period, or late-return policy revision could be created against a parent that another transaction had just archived.

## Server authority

Supported inventory mutations continue to require authenticated tenant context and `inventory:manage`. Late-return policy revisions keep their existing `inventory:read` plus `pricing:manage` authorization boundary.
...
Late-return policy revision authoring first takes the shared unit-type lifecycle lock, then its narrower policy-version lock, and only then re-reads the tenant unit type as `ACTIVE`. The parent-first order lets policy version retries keep their existing serialization while also preventing a newly authored revision from racing unit-type archival.
...
`20260919212500-rental-parent-lifecycle-authority` installs the shared parent locks and the physical-unit/rate-period guards. `20260919222000-rental-late-return-policy-parent-authority` extends the same unit-type lifecycle boundary to fresh late-return policy revisions without rewriting the older append-only policy trigger.
...
Fresh late-return policy revisions use an alphabetically early `BEFORE INSERT` guard to take the shared unit-type lifecycle lock and require the exact tenant-owned parent to remain `ACTIVE`. The existing policy-authority trigger then takes its dedicated policy lock and continues to own append-only revision numbering, currency agreement, fee bounds, and database-authored timestamps. UPDATE and DELETE remain rejected by that existing append-only authority.
...
- if the child/configuration write wins the lock, it commits against an active parent before archival can continue;
- if parent archival wins, the later child/configuration write re-reads the archived parent and fails closed.
...
Archiving a parent does not delete historical units, bookings, rate periods, late-return policy revisions, or evidence. Archived physical units and retained policy revisions may continue to reference their historical parents. Archival blocks only fresh authority that requires an active parent. This contract also does not add location reopening, unit-type reopening, customer pickup/drop-off semantics, one-way movement, transfer pricing, or delivery behavior.

## Validation

`src/server/inventory/rental-lock-domain.test.ts` protects the lock-key namespaces.

`scripts/rental-parent-lifecycle-source-contract.test.mjs` protects application lock/revalidation ordering, database child/configuration/parent guards, tenant scoping, deterministic parent lock ordering, and the retained-history boundary.

`scripts/rental-late-return-policy-source-contract.test.mjs` additionally protects the late-return policy's shared parent lifecycle serialization while retaining its independent policy-version authority.
