...
SF therefore does not persist locator-less `13034` as though Sync were already proven necessary. The provider classifier recognizes only a bounded homogeneous `13034` family from complete newer error evidence with canonical string `SourceCode=13034`, canonical `category=UNKNOWN`, and matching HTTP status, but the adapter-to-core settlement mapper normalizes that no-confirmation/no-recovery outcome to `AMBIGUOUS / TRAVELPORT_SELL_UNCERTAIN`. It carries no supplier confirmation, Travelport PNR Locator, or Sync recovery reference and is never retry authority.
