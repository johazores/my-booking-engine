...
SF therefore treats `page`, `pageSize`, `totalPages`, and `totalItems` as bounded machine authority rather than permissive presentation metadata.

## Response contract

The public Travelport adapter now rejects successful SearchComplete responses when pagination metadata is internally impossible within SF's documented five-page / 100-items-per-page boundary.
...
- a zero-page empty-result compatibility shape is valid only as page 1; and
...
Opaque `paginationToken` authority remains exact: no leading/trailing whitespace, no ASCII controls, and a maximum length of 4,096 characters. Follow-on callers remain limited to page numbers 2 through 5 and the provider still verifies that a continuation response reports the requested page number.
...
These layers are intentionally independent. Direct callers of the provider should not be able to observe impossible SearchComplete pagination metadata merely because the higher-level complete-search collector would later reject it.
...
- a current page greater than the reported total page count; and
- total item counts that exceed the capacity of the reported number of pages.

Coverage also preserves the existing valid empty first-page compatibility shape. The dependency-free pagination contract pins the new lower-bound and cross-field invariants at the public adapter boundary.
