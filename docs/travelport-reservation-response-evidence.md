...
Known-locator recovery calls this parser only after the transport returned a successful HTTP response. A successful `ReservationResponse` that also presents a top-level `ErrorResponse` is contradictory provider evidence and fails closed to `INVALID_RESPONSE`; response-family presence is based on property presence, so explicit `ErrorResponse: null` cannot disappear beside the reservation half and be promoted to `FOUND`.

Travelport's shared `ReservationResponse` contract can also include a `Result` object that carries warning or error evidence. Genuine omission of `Result` remains compatible, but a present `Result` must be a structured object. SF accepts warning-only `Result` data as non-authoritative context only when the warning evidence itself is structurally safe: at most one supported `Warning`/defensive `Warnings` array may be present, the collection is bounded, and every warning contains a bounded single-line `Message`. Conflicting warning families, explicit-null warning fields, non-array warning containers, oversized collections, or malformed warning messages invalidate positive Retrieve authority instead of being ignored. Any present embedded `Result.Error` evidence, including explicit `null`, and the defensive unsupported plural `Result.Errors` shape likewise fail closed. A valid-looking reservation, PNR, and hotel segment cannot hide contradictory or malformed embedded provider result evidence.
...
Travelport can return multi-content reservations containing non-hospitality products and can also return an explicit placeholder passive hotel offer alongside the active hotel offer. SF ignores well-formed non-hospitality products for Stays identity and excludes only offers explicitly marked `passiveOfferInd=true` from active hotel-segment cardinality. When the passive indicator is present it must be a boolean; explicit JSON `null` and every other non-boolean value are malformed, while genuine omission preserves the existing non-passive interpretation. An explicitly passive offer is excluded before its intentionally incomplete `Product` body is inspected. Every non-passive or unclassified offer must instead expose a bounded, non-empty `Product` array whose entries are structured objects with a bounded `@type`; structurally unreadable active offer/product evidence fails closed rather than being silently skipped beside one valid hotel segment. The current single-room known-locator recovery workflow then requires exactly one non-passive `ProductHospitality` segment, and that active segment must exactly match the durable property, stay dates, room quantity, and guest count. A second non-passive or unclassified, mismatched, or malformed `ProductHospitality` segment remains contradictory reservation evidence and fails closed instead of being filtered away because another active hotel segment matches.

When a durable reservation expectation is supplied, every returned offer must also expose one bounded, non-empty, unique `id`. Those IDs form the complete namespace for receipt `OfferRef` evidence. Any receipt `OfferRef` presented to known-locator recovery must be a bounded non-empty array whose members resolve to returned offer IDs; mixed active/passive references fail closed. This lets SF prove segment ownership before the narrow documented passive-placeholder rule and passive-owned durable identity/lifecycle filtering run, while reservation-level receipts such as the Travelport PNR remain valid without `OfferRef`.

A non-placeholder receipt scoped exclusively to explicit passive offers is first validated by the shared Stays receipt inspector. Malformed or contradictory Stays evidence still fails closed. Valid Travelport PNR, supplier Confirmation Number, and supplier cancellation lifecycle evidence is then excluded from the active reservation authority set when `OfferRef` proves it belongs only to a passive segment. A passive PNR therefore cannot satisfy the requested active locator or create false duplicate-PNR ambiguity beside the reservation-level PNR; a passive supplier confirmation cannot become the active supplier confirmation; and passive cancellation evidence cannot make the active segment appear cancelled. Supported non-authoritative shared-model evidence remains subject to the shared inspector without being promoted into those active durable fields.

The narrow documented passive-placeholder receipt is discarded only when `Confirmation.Locator` is genuinely omitted. Explicit `Locator: null` is malformed presentation, not proof of the placeholder shape, and therefore falls through to normal receipt validation where it fails closed.

This Retrieve-only allowance follows Travelport's documented active-plus-placeholder-passive response shape and its rule that information related only to a segment is identified with that offer. It does not change Create Reservation or Booking.com Sync success semantics: those write paths continue to use the independent stricter commercial classifier and do not inherit the recovery parser's passive-offer authority filtering.
...
Known-locator Hotel Retrieve supplies `expectedProviderReservationReference`, so the returned active/reservation-level Travelport PNR Locator must equal the requested locator exactly. Retrieve also rebinds property, stay dates, room quantity, and guest count to the durable reservation request.

The low-level parser can still inspect historical provider records when active-state confirmation is not requested. That is deliberately separate from provider-neutral recovery authority. `TravelportStaysReservationRecoveryProvider` sets `requireConfirmedTravelportReceipt=true` before it can emit `FOUND`, so the current active/reservation-level Travelport PNR receipt must be `Confirmed`, every active supplier Confirmation Number receipt that is presented must be `Confirmed`, and active/reservation-level supplier `Cancellation Number` evidence fails closed. Travelport's cancellation response can keep the PNR receipt `Confirmed` while changing the supplier receipt to `Cancellation Number` / `Cancelled`; checking the PNR status alone is therefore insufficient to prove an active reservation.
...
- a genuinely omitted or structured `ReservationResponse.Result`, with no present `Result.Error`/`Errors` evidence and only structurally valid bounded warning arrays when warnings are present;
...
A body containing both top-level `ReservationResponse` and `ErrorResponse`, including an explicitly null competing sibling, neither envelope, an error envelope on 2xx/3xx, a reservation envelope on non-2xx, or malformed/present `ReservationResponse.Result` error evidence never grants success, review, definitive failure, or Sync/recovery authority. Contradictory dual-envelope bodies also do not choose one provider trace identifier as durable correlation evidence.
...
Sync confirms only when its response proves the exact expected property/stay/occupancy, the original supplier confirmation, and exactly one confirmed Travelport PNR Locator. Because Sync reuses the Create commercial-response classifier, contradictory top-level response/error envelopes, malformed active offer/product structure, and malformed or embedded-error `ReservationResponse.Result` evidence fail closed before the documented Sync-only missing-`locatorType` normalization can grant confirmation authority. Pre-provider deterministic failure is retryable only when no Sync provider marker exists; after the marker, uncertainty remains non-retryable `AMBIGUOUS`.
...
Focused tests cover top-level envelope exclusivity/HTTP-class coherence including explicit-null competing siblings, absent-not-null `ReservationResponse.Result` and error/warning members, malformed/conflicting/oversized warning evidence rejection while preserving bounded warning-only responses, canonical reciprocal Stays locator pairing, PNR-locator identity, supplier locator-type semantics, active-vs-historical Retrieve state, supplier cancellation evidence, exact receipt cardinality including repeated identical and malformed/unconfirmed relevant provider/supplier receipts, known-locator active-hospitality cardinality including the documented active-plus-passive placeholder response and malformed or explicit-null passive indicators, bounded unique offer-ID and receipt-`OfferRef` scoping, passive-owned PNR/supplier-confirmation/cancellation authority exclusion, genuine omission versus explicit-null passive-placeholder Locator evidence, malformed passive durable-evidence rejection, malformed active/unclassified offer/product structure that cannot be skipped beside valid hotel evidence, contradictory extra/malformed non-passive hotel segments, preservation of well-formed non-hospitality multi-content products, known-locator exact-reference checks, unsafe locator/correlation values, Create success/ambiguity, Booking.com Sync recovery authority, review-required settlement, one-time accepted-review consumption, and privacy minimization.
