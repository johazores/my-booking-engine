...
## Persistence write-scope contract

The expired Stripe recovery persistence boundary is documented in `docs/stripe-commercial-amendment-recovery-write-scope.md`. Recovery executor, recovery Checkout, polling reconciliation, and both signed webhook families retain tenant, booking, amendment, operation, provider, lifecycle, exact-money, settlement-source, and verified webhook evidence at the final Prisma mutation instead of falling back to ID-only writes after an earlier scoped read. `scripts/stripe-commercial-amendment-recovery-write-scope.test.mjs` guards that boundary without replacing database/provider validation.

## Validation expectations

Dependency-free tests cover recovery decisions, Stripe recovery operation identity, recovery webhook ownership, Checkout reconciliation, product transport state/attempt-key mapping, post-settlement apply-failure routing, and the recovery persistence write-scope contract. Database validation remains required for advisory-lock ordering, tenant predicates, idempotency races, provider persistence, signed webhook/polling concurrency, target-hold release, apply-failure recovery routing, amendment terminalization, and payment/audit persistence. Do not claim those PostgreSQL paths passed unless the guarded disposable database suite runs against an explicitly confirmed disposable target.