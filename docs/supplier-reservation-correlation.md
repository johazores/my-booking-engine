...
`HospitalitySupplierReservationRecoveryProvider.retrieveReservation` receives the known provider reservation reference, the durable current attempt UUID as `requestCorrelationId`, and a required server-owned `beforeProviderRequest` callback.
...
Dependency-free/source contracts verify durable correlation and marker ordering across Create, reviewed Create, Sync, and reconciliation. The known-locator contract specifically locks the ordering `deterministic validation, OAuth, and transport-policy preflight -> durable marker -> real Retrieve`, requires the coordinator to reject unmarked provider results rather than manufacturing transport evidence afterward, and requires provider-neutral `NOT_FOUND` retry authority to be explicit rather than inferred from a result enum alone. The Travelport negative-evidence contract additionally locks `13061` status/category/message/envelope validation and prevents generic HTTP status or contradictory provider text from becoming retry authority. The provider-request marker contract also verifies the live tenant integration/provider/credential/capability recheck occurs before a new marker is written.
