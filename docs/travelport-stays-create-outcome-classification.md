...
Travelport's Stays error reference documents `13034` as an `UNKNOWN` server-side outcome at HTTP `500`. The Stays guide is more specific about its operational meaning: the same unconfirmed-supplier error can represent either a timeout where no Booking.com sell occurred or a timeout where Booking.com sold the room but Travelport did not receive the response. The branch is determined by whether the traveler receives the Booking.com confirmation email.

That means `13034` by itself proves neither safe retry nor Sync authority. SF's provider classifier recognizes only a structurally valid homogeneous `13034` family from Travelport's newer SourceCode-bearing envelope. The current documented JSON structure must be present: `Result.@type=Result`, each error must be `@type=ErrorDetail`, `SourceID` and `Message` must be bounded non-empty strings, `SourceCode` must be the canonical decimal string representation used by the newer JSON error shape, the serialized lowercase `category` member must carry the canonical uppercase value `UNKNOWN`, no competing plural-error or warning family may coexist in that Result, and the numeric body `StatusCode` and actual HTTP response must both be exactly `500`. Numeric SourceCode coercion, whitespace-normalized machine tokens, lowercased category values, or a syntactically complete `13034` payload at HTTP `400`, `404`, or another status do not gain commercial authority. Because even the documented HTTP 500 error response contains no verified supplier confirmation or recovery reference, `travelportStaysCreateOutcomeToSubmissionOutcome` normalizes the durable failure code to `TRAVELPORT_SELL_UNCERTAIN` while keeping the operation `AMBIGUOUS`.
...
Travelport's Stays error contract uses two provisioned formats. The older format contains only `StatusCode` and `Message`; it cannot provide SourceCode/category authority. The newer format adds `SourceID`, `SourceCode`, and `Category` conceptually; Travelport's current JSON example serializes `Result.@type=Result`, each SourceCode-bearing error as `@type=ErrorDetail`, `SourceCode` as a quoted decimal string, and the category field itself as lowercase `category` with uppercase machine values. SF uses that newer structural and decision-bearing evidence narrowly.
...
- every error has the documented lowercase `category` member carrying the canonical uppercase value `VALIDATION`;
- every `SourceCode` is a canonical decimal string in SF's reviewed no-sell validation allowlist; and
- exactly one unique source code is present.

A SourceCode-bearing envelope with missing or malformed discriminator, SourceID, Message, lowercase category member, canonical category value, SourceCode representation, or StatusCode evidence is not treated as a legacy response. It is incomplete newer evidence and remains ambiguous. The same fail-closed rule applies when `Errors`, `Warning`, or `Warnings` competes with the singular `Error` family. SF does not accept an uppercase JSON `Category` alias, numeric SourceCode coercion, category case folding, or surrounding-whitespace normalization, so non-canonical machine representations cannot be promoted into retry, review, definitive no-sell, or recovery authority.
...
The presence of a top-level `ErrorResponse` cannot be masked by confirmation-looking data. Malformed or oversized error collections fail closed. Source-code decisions require the complete current newer shape: `Result.@type=Result`, `Error.@type=ErrorDetail`, bounded `SourceID` and `Message`, numeric HTTP-consistent `StatusCode`, canonical decimal-string `SourceCode`, and the lowercase `category` member with a canonical uppercase value, with no competing error/warning family in Result. The older StatusCode/Message-only format does not grant source-code-based retry, review, or recovery authority.
