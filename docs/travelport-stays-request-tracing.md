...
Request bodies have a second fail-closed transport shape without turning the transport into a provider business-schema parser. When an OAuth body is present, it must remain the adapter-owned `URLSearchParams` password-grant form with exactly one `grant_type=password`, `username`, `password`, `client_id`, and `client_secret`, using the same field-length and whitespace bounds as Travelport credential normalization and exact `application/x-www-form-urlencoded` media type. When a Stays `POST` body is present, it must remain an in-memory JSON object string with exact `application/json` media type and at most 4 MiB of UTF-8 payload; Stays `GET` requests cannot carry a body. An inherited `Request` body arrives as a stream and therefore fails this representation contract unless the caller explicitly supplies a reviewed `init.body`. The wrapper checks shape and size without parsing, copying, hashing, logging, or retaining the JSON payload, which is important because Create can contain ephemeral FormOfPayment data. Provider-specific request schema and commercial authority remain owned by the adapters/coordinators.
