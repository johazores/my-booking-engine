...
Before the shared Stays receipt inspector runs, Retrieve excludes a receipt only when all of that receipt's bounded `OfferRef` values point to those explicitly passive offer IDs. This keeps Travelport's documented locator-less AK placeholder receipt from invalidating an otherwise authoritative active reservation response.
...
This means passive segment evidence can be ignored only because the provider itself scoped it exclusively to an explicitly passive offer. It cannot be used to hide malformed or contradictory active reservation evidence.
