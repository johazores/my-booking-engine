...
Tenant-owned child-resource reads bind `organizationId` plus the resource identifier. Mutable production services must also retain the relevant ownership and lifecycle preconditions at their persistence boundary rather than relying only on a preceding scoped read. The reviewed core administration/customer, non-hospitality inventory, hospitality inventory/pricing/availability, authenticated Stripe payment/refund initiation and reconciliation, core signed Stripe webhook, and public Stripe Checkout boundaries now have dedicated source contracts for that write-time ownership rule. Commercial-amendment webhook/settlement flows, invoice, adjustment-note, supplier, and similar state-machine writes remain separate focused review areas rather than being claimed as covered by a generic predicate sweep.

For child rows covered by the current write-scope contracts, final mutations repeat `organizationId` and the relevant resource/lifecycle scope when the model supports it. The `Organization` row is the tenant root and therefore has no separate tenant foreign key; root settings, branding, and archive mutations pin the exact server-authorized organization ID and the expected active/non-deleted lifecycle state. See `docs/core-tenant-write-scope.md`, `docs/non-hospitality-inventory-tenant-write-scope.md`, `docs/hospitality-tenant-write-scope.md`, `docs/stripe-payment-initiation-write-scope.md`, `docs/stripe-reconciliation-write-scope.md`, `docs/stripe-webhook-write-scope.md`, and `docs/public-stripe-checkout-write-scope.md` for the reviewed persistence boundaries.
...
- external/commercial writes additionally preserve their provider, idempotency, locking, reconciliation, and state-machine contracts and are reviewed with domain-specific regression coverage
