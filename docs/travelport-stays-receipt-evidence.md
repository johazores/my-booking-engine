...
- Travelport, Supplier, or Agency Stays contexts cannot omit `locatorType`, including on `ReceiptCancellation`; this does not ban an explicitly supplied generic non-Stays locator family such as `Travelport + Locator`, and a Stays locator type cannot be presented without its source context;
...
Travelport's current Sync response example can omit `locatorType` from the confirmed Travelport receipt. SF keeps that exception narrowly scoped to `travelport-stays-reservation-sync-domain.ts`. The adapter copies and annotates the locator as `PNR Locator` only when the missing-type receipt matches the documented reservation-level hospitality shape: `ReceiptConfirmation`, `ConfirmationHold`, no `OfferRef`, `sourceContext=Travelport`, and `OfferStatusHospitality` with `Status=Confirmed`. Explicit locator types are never rewritten, and partial/malformed lookalikes do not receive the compatibility normalization.
...
Focused tests cover shared normalization, supplier cancellation evidence, self-identifying Stays `ReceiptCancellation` normalization, unrelated shared-model cancellation compatibility, recognized Stays cancellation contexts that omit `locatorType`, malformed/contradictory Stays cancellation rejection, reciprocal source-context/locator-family pairing, documented Booking.com `Supplier + Pin code` compatibility without durable promotion, malformed/foreign-context PIN rejection, explicit contradictory `ConfirmationHold` / `CancellationHold` / `OfferStatusHospitality` discriminator rejection, bounded unrelated multi-content compatibility, the documented Sync-only missing-locator-type exception including rejection of malformed discriminator/status/offer-scoped lookalikes, end-to-end Create/Retrieve partial cancellation rejection, end-to-end Create/Retrieve malformed sibling rejection, and active Create/Retrieve cancellation rejection.

A dependency-free source contract requires Retrieve and Create/Sync to keep using the same inspector, protects the Travelport/Supplier/Agency locator-family rules including the narrow non-durable Booking.com PIN pair, requires Stays `ReceiptCancellation` to reject a recognized Stays source context that omits `locatorType`, requires Stays cancellation lifecycle evidence to flow into the existing cancellation rejection boundary rather than being blindly skipped, protects explicit hospitality discriminator checks, and checks that payment-card/form-of-payment fields are not part of the normalized receipt module.
