...
`scripts/rental-hold-database-clock-authority-source-contract.test.mjs` protects the PostgreSQL clock reads, idempotent replay ordering, database-derived hold timestamps/expiry, effective-list/review/release decisions, and the related inventory-mutation active-hold checks.
