...
This passive-placeholder allowance belongs only to known-locator Retrieve identity. Create Reservation and Booking.com Sync continue to use the independent commercial-write classifier and keep their stricter response cardinality rules; recovery parsing does not broaden external-write confirmation authority. The commercial classifier now applies the same fail-closed structural readability rule to all returned offers/products, but it does not discard passive offers before that validation.
