...
Commercial price-decrease refunds use a separate amendment-owned boundary rather than the generic booking-refund endpoints. `refundStripeHospitalityBookingCommercialAmendment` requires both `booking:manage` and `payment:manage`, tenant-scopes the booking and prepared amendment, and derives the exact Stripe settlement source and amount from the complete payment ledger plus the immutable amendment delta. It writes `commercialAmendmentId` and `sourceProviderReference`; the caller cannot choose either money or provider source.

Before the Stripe write, the amendment refund is persisted as `AMBIGUOUS` with a deterministic internal claim reference. This is intentional: generic Stripe polling and signed refund webhook finalizers own ordinary `PENDING` booking refunds and can update `HospitalityBooking.paymentStatus`. Amendment-owned money must not cross that generic finalization path because the booking snapshot and prepared `bookingVersion` must remain unchanged until the commercial amendment's final serializable apply transaction.

Exact retries re-derive the same amendment execution decision and reuse the same Stripe idempotency key. Definitive provider failure closes only an unresolved internal claim as `FAILED`; transport/time-out uncertainty stays `AMBIGUOUS`. A non-final provider response is also `AMBIGUOUS` but retains the real `re_` refund reference for read-only provider-truth recovery.

`reconcileStripeHospitalityBookingCommercialAmendmentRefund` reconciles only an amendment-attributed `AMBIGUOUS` refund with a real Stripe refund reference. It verifies organization, booking, amendment, transaction, persisted fingerprint, PaymentIntent source, currency, and amount against Stripe before changing only the payment transaction and audit evidence. It never updates booking payment/commercial state. Successful source-by-source amendment refunds become eligible for the existing final apply boundary only after the amendment settlement reconciler reports `READY_TO_APPLY`.

Signed refund callbacks now use the same isolated amendment-owned rule when the `AMBIGUOUS` row already persists the exact real `re_` provider reference. The standard tenant webhook verifier first authenticates and records the event; `finalizeVerifiedStripeCommercialAmendmentWebhook` then requires the exact verified event hash, refund reference, source PaymentIntent, amendment ownership, currency, amount, and refund fingerprint before updating only amendment payment evidence. It never updates booking payment/commercial state. Internal pre-reference `sf_claim_*` rows are intentionally not guessed from a callback; exact retry/polling/operator recovery remains authoritative for those cases. See `docs/stripe-commercial-amendment-webhooks.md`.

Commercial amendment Stripe refunds remain internal only; no API route or primary UI action exposes them yet. Stripe additional-charge execution and provider-known webhook recovery are also implemented internally. Customer-facing additional-charge collection/authentication and expired-hold/payment compensation remain open production dependencies before user-facing commercial adjustment settlement can be enabled.

## Validation

The standard payment unit-test coverage includes Stripe refund domain, reconciliation, webhook-domain, settlement-source attribution, refund allocation/execution planning, whole-booking refund-state reconciliation, refund-availability behavior, and amendment-owned Stripe refund claim/reconciliation rules. Coverage includes exact minor-unit parsing, provider-result mapping, read-only refund retrieval, provider-state mapping, source/money mismatch rejection, refund-object parsing, exact-reference preference, internal-claim binding, ambiguity/money mismatch rejection, per-source balance reconciliation, deterministic source selection, multi-source progression, legacy multi-source fail-closed behavior, and amendment claim isolation from generic pending-payment finalizers.

`booking-commercial-amendment-stripe-webhook-domain.test.ts` additionally covers exact amendment refund/PaymentIntent provider-reference selection, exact source/money enforcement, duplicate-reference ambiguity, and the deliberate refusal to bind a signed callback to an internal pre-reference amendment claim.

The guarded PostgreSQL runner includes `stripe-refund-webhook.integration.ts`. Its existing signed-callback scenario verifies provider-reference binding, finalization, exact-redelivery idempotency, and altered-event rejection. Source-aware multi-settlement behavior now exists in the application boundaries but still requires the disposable PostgreSQL suite to be executed, and the integration scenario should be expanded when the database test environment is available to exercise two successful Stripe settlements end to end. Amendment-owned Stripe refund persistence/locking and verified-event promotion also still require database-backed validation, including concurrent polling/webhook finalization.
