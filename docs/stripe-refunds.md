...
Commercial amendment Stripe refunds are internal only; no API route or primary UI action exposes them yet. Signed webhook finalization for amendment-owned refunds, Stripe additional-charge collection, and expired-hold compensation/recovery remain open production dependencies before user-facing commercial adjustment settlement can be enabled.
