...
Schema-version-6 cancellation-after-amendment snapshots already freeze an ordered set of exact cancellation refund transaction IDs, ordinals, amounts, and timestamps. Historical terminal refund authority now reloads only those exact tenant-owned rows and re-proves structural identity, source attribution, money, and chronology; it no longer depends on the current `PaymentTransaction.status`. Reconciliation still maps the frozen IDs to current refund status and records per-document `SETTLEMENT_DRIFT`. Missing refund rows remain source/integrity failures rather than being mislabeled as provider lifecycle drift.

This separation applies to the terminal schema-version-6 refund leg only. The commercial predecessor chain still contains schemas 2 through 5, whose snapshots did not freeze issue-time settlement authority. Commercial-amendment schemas 2 through 5 still need a versioned issue-time settlement-evidence contract before their historical read authority can be safely separated from later mutable provider-status changes. Reconciliation does not infer historical transaction membership that those snapshots did not freeze.
...
The current reconciliation boundary now includes increasing/decreasing mixed-direction commercial adjustments, terminal cancellation after commercial amendments, per-document current refund drift for schemas 1 and 6, and durable secret-safe failure occurrence counts in reconciliation history. Remaining work includes versioned issue-time settlement evidence for commercial-amendment schemas 2 through 5, mixed/partial/non-standard-GST adjustment rules, generic correction/void/reissue, broader booking-linked customer-data disposal/de-identification beyond the safe bookingless customer-profile boundary, durable customer re-authentication/email delivery, Unicode-safe deterministic PDF fonts, live Node 24/Prisma/PostgreSQL validation, and legal review.
