...
Schema-version-6 cancellation-after-amendment snapshots already freeze an ordered set of exact cancellation refund transaction IDs. Reconciliation now uses those frozen IDs to identify current refund-status drift per affected legal document as well. This is observability only: it does not weaken the existing schema-version-6 historical read authority or the commercial predecessor-chain checks. Missing refund rows remain source/integrity failures rather than being mislabeled as provider lifecycle drift.

A settlement-drift result never rewrites, hides, voids, or deletes the issued adjustment note and does not invent a correction or replacement-refund workflow. Commercial-amendment schemas 2 through 5 still need a versioned issue-time settlement-evidence contract before their historical read authority can be safely separated from later mutable provider-status changes; reconciliation does not infer historical transaction membership that those snapshots did not freeze.
...
The current reconciliation boundary now includes increasing/decreasing mixed-direction commercial adjustments, terminal cancellation after commercial amendments, per-document current refund drift for schemas 1 and 6, and durable secret-safe failure occurrence counts in reconciliation history. Remaining work includes mixed/partial/non-standard-GST adjustment rules, generic correction/void/reissue, broader booking-linked customer-data disposal/de-identification beyond the safe bookingless customer-profile boundary, durable customer re-authentication/email delivery, Unicode-safe deterministic PDF fonts, live Node 24/Prisma/PostgreSQL validation, and legal review. Commercial-amendment schemas 2 through 5 and schema-version-6 historical read authority still require a deliberate versioned issue-time settlement-evidence design before all mutable provider-status regressions can be separated safely from immutable legal evidence in the same way as schema version 1.