...
A completed reconciliation writes one tenant-scoped `AuditEvent` summary using `payment.tax-document-reconciliation.completed`. The run samples PostgreSQL `date_trunc('milliseconds', clock_timestamp())` inside the same `RepeatableRead` transaction and reuses that database-observed value for both the report `checkedAt` and `AuditEvent.createdAt`; application-node time is not reconciliation chronology authority. New audit schema version 3 stores only jurisdiction, status, UTC check time, exact document counts, normalized failure codes, and occurrence counts, and history accepts its database-clock guarantee only when `AuditEvent.createdAt` exactly matches report `checkedAt`. It deliberately excludes document numbers, customer PII, provider/payment references, credentials, raw snapshots, and fingerprints. Historical schema version 2 summaries remain readable with their failure counts but predate the event-metadata clock-binding guarantee. Historical schema version 1 summaries also remain readable; because they stored only unique failure codes, SF does not invent occurrence counts for those older records.
