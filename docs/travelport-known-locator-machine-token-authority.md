...
This boundary applies to `parseTravelportStaysReservationResponse` when the durable reservation expectation is supplied. The expectation's opaque supplier property reference is now held to the same exact-machine principle before Retrieve starts. This does not enable the Travelport `reservation` capability and does not broaden Create Reservation or Booking.com Sync behavior.

## Exact-token rule

The Retrieve parser requires provider machine strings to already be canonical. The common token guard rejects:
...
## Expected property-reference rule

Before known-locator response comparison, the shared `decodeTravelportStaysPropertyReference` boundary requires the expected supplier property reference itself to be one exact canonical base64url token. It rejects outer padding/control characters and rejects alternate base64url spellings that decode to the same bytes. The decoded `TVPT` chain/property codes are also validated without trimming.

This closes the input side of the same identity boundary: both the durable expected property identity and the provider response property identity must be canonical before they are compared. The same shared expectation decoder is also used by initial Create, reviewed Create, and Booking.com Sync, so the exact property-reference rule is consistent across reservation write and recovery paths. See `docs/travelport-reservation-property-reference-authority.md`.
...
The shared receipt inspector already applies the same exact-token principle to Stays-owned locator, status, discriminator, source, and supported `OfferRef` evidence. The expected-property hardening and surrounding known-locator response/segment parser now use the same fail-closed principle on both sides of the identity comparison.
...
Supplier-property-reference behavior coverage additionally rejects outer whitespace/controls, padded/control-bearing decoded chain/property values, and non-canonical base64url aliases. A dependency-free source contract pins both the Retrieve exact-token guard and canonical expectation-reference rule while keeping warning text on its separate bounded-text path.
