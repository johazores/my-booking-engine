...
Commercial-amendment schemas 2 through 5 still lack a versioned immutable issue-time settlement-evidence contract, so their historical authority can still depend on later mutable provider-status changes; schema version 6 can inherit that limitation through its commercial predecessor chain. Mixed taxability, partial/non-standard-GST adjustments, generic correction/void/reissue, other jurisdictions, durable customer re-authentication and email/resend, universal Unicode-safe PDF rendering, reviewed disposal/de-identification, complete Node 24/Prisma/PostgreSQL production execution, and jurisdiction/legal review remain separate production work and fail closed until implemented.
