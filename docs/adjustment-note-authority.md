...
For newly issued commercial adjustment notes, payment settlement is re-proved stepwise from immutable issue-time settlement evidence captured in the same transaction as the legal document. Provider lifecycle changes after issuance therefore do not redefine historical settlement authority for those notes. Pre-migration commercial notes are intentionally not backfilled because their original provider status cannot be reconstructed truthfully; only that leading legacy prefix continues to use the complete bounded retained-payment fallback. Once frozen evidence begins in a source chain, every later commercial note must also have it or historical verification fails closed.

Rows fail closed on unsupported direction/schema combinations, gaps, forks, duplicate authority, cross-tenant/source evidence, baseline drift, target-pricing drift, incomplete legal payment history, malformed or missing frozen evidence after capture begins, non-standard GST, unresolved/conflicting settlement, or an incorrect net settled amount.
...
Historical schema-version-6 authority does **not** re-run cancellation readiness against mutable current booking or payment status. `hospitality-cancellation-after-amendment-adjustment-authority-service.ts` reloads the tenant-scoped verified commercial predecessor chain and source invoice, then reloads only the exact refund transaction IDs frozen into the immutable cancellation snapshot. It re-proves refund identity, tenant/booking scope, non-commercial refund shape, source attribution, AUD money, timestamps, chronology, and the exact frozen total while deliberately ignoring current `PaymentTransaction.status`. Missing, duplicate, or structurally drifted rows fail closed; later provider lifecycle status drift is reported separately by tax-document reconciliation.

`issueHospitalityCancellationAfterAmendmentAdjustmentNote` still re-derives current cancellation readiness from complete bounded provider-neutral payment evidence inside the serializable source-chain lock before issuance. It allocates the shared AU adjustment-note sequence, persists canonical schema-version-6 evidence, and immediately verifies the created immutable authority in the same transaction.

PostgreSQL now independently guards the frozen schema-version-6 refund-membership shape as well. `sf_hospitality_v6_refund_authorities_valid` plus `hospitality_adj_notes_v6_refund_authorities_check` reject malformed, duplicate, reordered, non-positive, chronologically invalid, unexpected-field, issue-time-mismatched, or wrong-total refund-authority JSON even for direct SQL/ORM inserts. The guard intentionally does not freeze mutable provider status or references. See `docs/cancellation-refund-authority-database-integrity.md`.
...
New schema-version-2-through-5 commercial adjustment notes now carry immutable issue-time settlement evidence. Historical pre-migration commercial notes remain a deliberate legacy limitation because SF will not fabricate provider status that was not frozen when those documents were issued. Mixed taxability, partial/non-standard-GST adjustments, generic correction/void/reissue, other jurisdictions, durable customer re-authentication and email/resend, universal Unicode-safe PDF rendering, reviewed disposal/de-identification, complete Node 24/Prisma/PostgreSQL production execution, and jurisdiction/legal review remain separate production work and fail closed until implemented.
