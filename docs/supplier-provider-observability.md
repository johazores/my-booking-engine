...
The coordinator also verifies that a known provider reservation reference exists and that the injected recovery adapter code matches the durable provider code. It then supplies the provider with a server-owned `beforeProviderRequest` callback. The provider must complete deterministic request validation, authentication, and no-I/O transport-policy preflight before invoking that callback. The callback rechecks the live tenant integration/provider/credential/capability authority, writes `providerRequestStartedAt`, and creates the provider-request observation immediately before the real Retrieve can start.

No provider-request completion record is emitted when the provider call never reaches that boundary, including provider-code mismatch, missing durable locator, deterministic request failure, authentication/preflight failure, or a provider implementation that returns without invoking the callback. Those cases remain represented by the durable supplier reservation state/audit boundary, and SF does not fabricate provider-start evidence after the provider returns.
...
The reconciliation coordinator normalizes provider correlation and `FOUND` supplier-confirmation fields before durable settlement. Supplier-confirmation completeness and identity are then evaluated through the shared provider-neutral evidence rule plus any provider-declared `FOUND` completeness requirement before either success observation is allowed. For negative recovery, the raw runtime supplier-confirmation field is deliberately inspected before optional normalization because a `NOT_FOUND` result must not hide contradictory sell evidence simply because TypeScript says the field should be absent.

For `FOUND`, a known durable supplier confirmation must be returned exactly. In addition, a recovery adapter can declare that every provider-neutral `FOUND` result requires a supplier confirmation before commercial settlement. Travelport declares that requirement because the supplier Confirmation Number is required for later hotel lifecycle operations. Therefore a missing Travelport supplier confirmation fails before an info-level `FOUND` observation regardless of the operation's prior ambiguity. The durable ledger records `SUPPLIER_CONFIRMATION_MISSING` when no supplier confirmation ever existed, or `SUPPLIER_CONFIRMATION_MISMATCH` when established supplier identity is missing/different, while preserving known reservation identity.

For provider-neutral `NOT_FOUND`, both durable and runtime supplier-confirmation evidence are contradiction checks. SF does not emit an info-level `NOT_FOUND`, erase known provider/supplier identity, or authorize another Create when a durable supplier confirmation exists. It also rejects a `NOT_FOUND` runtime object that unexpectedly presents any non-null supplier-confirmation value even when the ledger had none; otherwise a buggy adapter could convert evidence of a sell into retry authority. These cases produce a warning-level `failed / INVALID_RESPONSE` observation, while the durable operation records `SUPPLIER_CONFIRMATION_MISMATCH` and remains ambiguous. Only authoritative negative evidence with no durable supplier confirmation and no supplier confirmation in the result can reach the normal `NOT_FOUND` settlement path. Travelport itself still does not infer authoritative `NOT_FOUND` from a generic HTTP 404.
...
`scripts/supplier-reservation-provider-observability.test.mjs` covers the safe structured recovery record, normalization of unsafe log identifiers, malformed success-result fail-closed behavior, one-completion-only behavior, authority/provider-I/O ordering, durable attempt correlation, explicit `FOUND` / `NOT_FOUND` handling, fail-closed unrecognized provider results, and the requirement that supplier-confirmation evidence be accepted before either success observation. `scripts/supplier-reconciliation-provider-boundary-contract.test.mjs` additionally locks the immediate pre-I/O marker/observation boundary for known-locator recovery. `scripts/supplier-reservation-confirmation-completeness-contract.test.mjs` pins provider-declared `FOUND` confirmation completeness before success logging/settlement. `scripts/supplier-reservation-recovery-supplier-identity-contract.test.mjs` separately pins raw runtime supplier-evidence capture, durable settlement defense in depth, and the rule that supplier confirmation cannot be replaced, erased, or smuggled through a negative recovery result.
