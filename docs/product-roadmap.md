...
The selected-offer create-authority ambiguity is now resolved in code without exposing Travelport models to the product domain. The read-only authority adapter repeats fresh Rules/offer review, remaps the selected SearchComplete rate to complete bounded v11 Availability results, requires one exact selected-rate match, and returns only a deterministic `authorityFingerprint`. Request fingerprint v2 binds that reviewed authority to the durable commercial request. The fingerprint is review evidence rather than a timeless sell token, so the same authority bridge must be repeated immediately before any future provider write. Live validation of that SearchComplete-to-Availability mapping against a provisioned non-production account remains required before reservation capability can be enabled.

The provider-neutral durable external-write foundation is implemented through tenant-owned supplier reservation operation/attempt persistence. It provides organization-scoped exact idempotency, accepted offer/Rules/reservation-payload/authority fingerprints, exact-money and stay/occupancy evidence, integration credential-version binding, serializable operation claims, database-clock crash-recovery leases, persisted provider/correlation evidence, and a fail-closed `AMBIGUOUS -> RECONCILING` state that forbids blind duplicate creates. Known-locator Travelport Hotel Retrieve is connected to that ledger through a provider-neutral reconciliation coordinator. `FOUND` can confirm only the exact durable Travelport locator and can persist the normalized supplier confirmation. Provider-neutral `NOT_FOUND` can make another create retryable only when an adapter supplies verified authoritative negative evidence for the exact locator that was queried; the current Travelport adapter does not infer `NOT_FOUND` from a generic HTTP 404 because public Retrieve documentation does not establish that status as authoritative reservation non-existence. Generic 404, malformed, or mismatched provider evidence therefore remains `AMBIGUOUS`. Locator-less uncertainty remains closed because the current Retrieve contract cannot prove provider truth without an aggregator locator.

Travelport `reservation` capability remains unadvertised and no staff/customer reserve action should be exposed until the SearchComplete-to-Availability mapping and real Create Reservation request/response behavior are validated in non-production, SF has a reviewed PCI-safe form-of-payment/guarantee strategy for the provisioned account, the real single-room create coordinator settles every provider outcome through the durable ledger, explicit price/guarantee-change decisions are handled safely, authoritative negative lookup semantics are established, and locator-less ambiguous-write recovery/correlation behavior is verified. Travelport documents the reference Create Reservation request as carrying traveler plus form-of-payment/payment data and explicitly forbids sending `acceptPriceChangeInd` or `acceptGuaranteeChangeInd` on the initial request, so SF must not weaken its existing no-raw-card application boundary or silently accept provider changes. Multi-room, modification, and cancellation capabilities must be verified independently rather than inferred from create support.
