...
The provider-neutral durable external-write foundation is now implemented through tenant-owned supplier reservation operation/attempt persistence. It provides organization-scoped exact idempotency, accepted offer/Rules/reservation-payload fingerprints, exact-money and stay/occupancy evidence, integration credential-version binding, serializable operation claims, persisted provider references/correlation evidence, and a fail-closed `AMBIGUOUS -> RECONCILING` recovery state that forbids blind duplicate creates. This ledger is intentionally separate from first-party `HospitalityBooking` inventory and does not itself call a supplier.

The next dependency is the real Travelport single-room reservation create plus provider-truth retrieval/reconciliation adapter wired through that ledger. Travelport `reservation` capability must remain unadvertised, and no staff/customer reserve action should be exposed, until create/retrieve semantics are verified and validated against a provisioned non-production account. Multi-room and retrieve/modify/cancel capability must be verified independently rather than inferred from the single-room Rules boundary.
