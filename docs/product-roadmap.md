...
Rental availability separately excludes explicit blocks, effective holds, and non-cancelled durable rental booking allocations under the physical-unit serialization boundary described in `docs/rental-inventory.md` and `docs/rental-booking-foundation.md`. Rental cancellation uses that same unit lock before changing the booking to terminal `CANCELLED`, after which the retained allocation no longer blocks live availability.
...
Rental booking management is not part of this hospitality management contract. Rental now implements its own staff inventory-release cancellation under tenant/booking/unit serialization while retaining the allocation and commercial evidence. Rental amendments/rescheduling, payment/deposit consequences, fulfillment changes, and any cancellation financial policy remain separate rental contracts rather than being inferred from hospitality behavior.
...
The tenant-owned tour/package and appointment inventory foundations remain infrastructure-only. Rental has progressed further: temporary availability protection, immutable rate evidence, server-side booking conversion authority, staff hold-to-booking confirmation, durable confirmed booking/allocation persistence, tenant-scoped booking history/detail, booked-inventory protection, terminal inventory-release cancellation, and rental-linked customer retention are implemented. The broad rental workflow remains incomplete until customer booking surfaces, amendment/rescheduling, payment/deposit, pickup/drop-off/return, fulfillment, and external synchronization receive their own production contracts.
