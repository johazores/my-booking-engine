...
The current server-only single-room Create coordinator repeats fresh offer/Rules/Availability/traveler/payment authority, claims a `CREATE` attempt, rechecks the exact integration and credential version, marks the durable provider-request boundary, invokes fixed Travelport Create Reservation, and settles confirmed, failed, ambiguous, or documented price/guarantee review outcomes through the durable ledger. The Booking.com Sync coordinator separately rebinds traveler authority, claims its own recovery-write attempt and provider marker, sends no form-of-payment, and confirms only when the original supplier confirmation and one Travelport locator return with the expected reservation identity. These write paths remain product-unreachable while `reservation` is unadvertised.

Documented Travelport price/guarantee no-sell responses now persist as dedicated `REVIEW_REQUIRED` state rather than ordinary failure. A tenant-authorized server-only acceptance boundary verifies the exact durable marked/completed Create attempt, explicitly matches the accepted change dimensions, repeats fresh SearchComplete/Rules/Availability/traveler/integration/payment authority, and persists bounded non-secret decision evidence plus an audit event. Normal retry remains blocked. The accepted decision is not yet consumed into Travelport's documented second Create request; a dedicated one-time claim/consumption path must preserve exactly-once provider-attempt authority, provider-request markers, ambiguity/recovery behavior, and send only the applicable acceptance query flag(s).

Travelport `reservation` capability remains unadvertised and no staff/customer reserve action should be exposed until all activation gates are satisfied: live SearchComplete → Rules → Availability → Create → Sync verification with provisioned non-production credentials; a reviewed PCI-safe form-of-payment/guarantee source appropriate for the account; the one-time accepted-review second-write consumption path; verified `13034`/locator-less correlation and retry semantics; and complete provider-truth recovery behavior. Travelport explicitly forbids sending `acceptPriceChangeInd` or `acceptGuaranteeChangeInd` on the initial request, so SF must not weaken its existing no-raw-card application boundary or silently accept provider changes. Multi-room, modification, and cancellation capabilities must be verified independently rather than inferred from create support.
