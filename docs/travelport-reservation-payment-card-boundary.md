...
Before the external source capability is invoked, SF now reads each allowed context field exactly once into a new frozen allowlisted snapshot. Caller-only metadata is not copied. Throwing context accessors and revoked proxies fail closed to a fixed non-retryable `INVALID_REQUEST` boundary instead of propagating caller-controlled diagnostics. Array capability objects are also rejected before acquisition. These rules keep the future source invocation bound to one stable tenant/reservation/integration/attempt identity even when a runtime caller is not a plain immutable object.
...
The same sanitization boundary covers source-result materialization. The card result is copied immediately into a new frozen allowlisted snapshot before it reaches the Travelport Create adapter. Only the declared card, billing-address, and telephone fields are copied; unrelated source metadata is discarded. Every sensitive top-level field and every accepted nested field is read exactly once. This prevents accessor/proxy results from returning one value for validation and another value later, and it prevents accessor failures from bypassing the fixed source-error message with source-controlled diagnostics.
...
The provider-neutral payment-authority layer intentionally remains provider-agnostic and keeps card codes opaque but bounded. Travelport-specific syntax is enforced only at the Travelport response and request boundaries. The low-level Create executor independently repeats the bounded exact-two-character and duplicate checks when handed payment authority directly, requires the ephemeral source card code itself to be exactly two uppercase alphanumeric characters, and requires that selected code to belong to the freshly validated accepted-card set. This keeps malformed short, oversized, padded, duplicate, or contradictory accepted-card authority from becoming a Travelport POST even if an internal caller bypasses the normal request-material coordinator.
...
The payment-source regression coverage verifies both sides of the capability boundary: non-sensitive execution context is a one-read frozen allowlist with fixed failure semantics, while accessor-backed card results are materialized into a frozen allowlisted snapshot, each accepted sensitive field is read once, extra metadata is not propagated, and both top-level and nested accessor failures are sanitized before adapter validation.
