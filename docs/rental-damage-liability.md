...
`RentalDamageLiabilityDecision` is append-only. PostgreSQL rejects update/delete, validates closed damage-case authority, currency, amount/outcome shape and deterministic idempotency, then authors chronology with `clock_timestamp()`.
