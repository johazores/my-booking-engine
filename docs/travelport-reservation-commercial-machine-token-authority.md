...
The production reservation transport now applies one response-machine-authority guard after exact trace binding and before any Create, Booking.com Sync, or known-locator compatibility classifier/parser receives a reservation body. Successful reservation identity fields cannot be trimmed into the expected chain/property/stay or offer/receipt identity. Result evidence now also requires exact documented discriminators before it can reach the compatibility classifiers: a present result type must be `Result`, each error entry must be `ErrorDetail`, a present warning type must be `Warning`, the legacy `Errors` and `Category` aliases are rejected, and a legacy payload `traceID` alias is rejected even when canonical `traceId` is also present. Error status codes must be integral HTTP-range values from 100 through 599; warning status codes, when present, must be integral values from 0 through 999. Explicit empty error or warning collections cannot stand in for absent evidence, and one result cannot carry both error and warning authority. Error `SourceCode`, `category`, type, and source evidence must remain canonical, and bounded warning text cannot gain Sync-recovery meaning through outer whitespace or hidden ASCII controls. The guard also repeats the existing response collection ceilings before traversing offer/product/receipt/error/warning authority.

Travelport's current Stays error documentation returns newer errors in `Result/Error` with `StatusCode`, `Message`, `SourceID`, `SourceCode`, and uppercase `category` values such as `VALIDATION`, `RETRY`, `TERMINAL`, or `UNKNOWN`. Its current JSON example serializes `SourceCode` as a quoted decimal string. Structured error entries must carry those machine fields rather than omitting, nulling, or changing their JSON representation. SF therefore treats the source code, status code, discriminator, source ID, message presence, and category as exact machine classification evidence instead of silently trimming, recasing, coercing a numeric JSON SourceCode into text, or accepting alternate field spellings. The downstream Create/Sync classifiers continue to own HTTP-status equality and documented source-code semantics; this response guard prevents malformed or representation-coerced machine evidence from reaching them. Provider messages remain non-persisted/non-logged; the bounded message check only prevents normalization-confusable text from participating in the small documented warning contract used by reservation recovery.
...
- rejection of normalization-confusable successful reservation identity, offer authority, property/stay evidence, receipt/locator evidence, commercial error source/category evidence, numeric SourceCode representation, and Sync-selecting warning text before downstream classification;
- exact `Result` / `ErrorDetail` / `Warning` discriminators, rejection of legacy `Errors`, `Category`, and `traceID` aliases, bounded integral error/warning status authority, rejection of explicit empty result collections, and rejection of conflicting error-plus-warning results;
