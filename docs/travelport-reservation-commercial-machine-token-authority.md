...
Travelport's current Stays error documentation returns newer errors in `Result/Error` with `StatusCode`, `Message`, `SourceID`, `SourceCode`, and uppercase `category` values such as `VALIDATION`, `RETRY`, `TERMINAL`, or `UNKNOWN`. Structured error entries must carry those machine fields rather than omitting or nulling them. SF therefore treats the source code, status code, discriminator, source ID, message presence, and category as exact machine classification evidence instead of silently trimming, recasing, or accepting alternate field spellings. The downstream Create/Sync classifiers continue to own HTTP-status equality and documented source-code semantics; this response guard only prevents malformed machine evidence from reaching them. Provider messages remain non-persisted/non-logged; the bounded message check only prevents normalization-confusable text from participating in the small documented warning contract used by reservation recovery.
