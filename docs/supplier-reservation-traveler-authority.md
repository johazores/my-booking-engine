...
The Create request mapper separately carries the freshly selected Availability `CatalogOfferingIdentifier` and exact non-secret `Payment` amount/indicators. It deliberately does **not** construct `FormOfPayment`, `PaymentCard`, card number, CVV/security code, cardholder, or billing-card data. Those fields remain behind the PCI-safe form-of-payment boundary. The separate provider request-text contract rejects ASCII controls and ill-formed Unicode in ephemeral cardholder and billing text before sensitive Create serialization.
...
See also `docs/travelport-reservation-request-text-authority.md` for the shared provider-bound ASCII-control and well-formed-Unicode policy applied to traveler identity/contact, supplier references, Sync authority, and sensitive Create single-line text.
