...
SF can generate deterministic A4 PDF copies of verified issued Australian hospitality tax invoices and the currently supported full-cancellation adjustment notes. PDF generation is a read projection only: it never creates, renumbers, corrects, or mutates legal-document authority.
...
The adjustment-note renderer consumes the customer-safe document derived from immutable `HospitalityIssuedAdjustmentNote.documentSnapshot` evidence after its material-column, fingerprint, issuer/recipient, source-tax-invoice, cancellation price-effect, and exact decrease checks have passed.
...
Authenticated tax-invoice and adjustment-note detail pages expose both **Download PDF** and **Print or save copy**.

The public booking tax-document history exposes **Download PDF** for each visible verified invoice and adjustment note. Download actions have busy/disabled states, keep the booking capability in the request body, download the response as a local file, and surface a specific safe error when legal text cannot be represented losslessly by the current PDF contract.
...
Dependency-free adjustment-note renderer tests cover byte-for-byte determinism, source-document identity, unsupported-script rejection, AUD-only scope, exact decrease reconciliation, full-cancellation before/after price semantics, and source-document chronology.

During tax-invoice implementation a generated fixture was parsed with local `pdfinfo`, which reported a valid PDF 1.4, A4 page, and expected page count. Adjustment-note PDF fixtures use the same deterministic PDF object contract and are validated separately by the focused renderer suite.
