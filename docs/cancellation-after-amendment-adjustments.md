...
The commercial predecessor chain remains separately verified. Newly issued schema-version-2-through-5 commercial adjustment notes now freeze a separate versioned issue-time settlement-evidence ledger in the same issuance transaction. Historical verification replays that provider-neutral frozen ledger, so later provider lifecycle changes do not rewrite their original legal authority. Existing pre-migration commercial notes are intentionally not backfilled because their issue-time provider status cannot be reconstructed truthfully; only a leading legacy prefix may use the bounded retained-payment fallback. Once frozen capture begins, every later commercial note in that source chain must also have frozen evidence or historical verification fails closed. A schema-version-6 terminal cancellation therefore inherits the legacy settlement limitation only when its verified predecessor chain includes that unavoidable pre-migration prefix.
...
Pre-migration schema-version-2-through-5 commercial notes remain an explicit legacy boundary because provider status that was not frozen at issuance cannot be fabricated later; no synthetic backfill is planned without authoritative historical evidence. Phase 12 still has separate work for mixed-taxability and partial/non-standard-GST adjustment semantics, generic correction/void/reissue, durable customer re-authentication and email/resend, universal Unicode-safe deterministic PDF rendering, reviewed disposal/de-identification, complete Node 24/Prisma/PostgreSQL execution, and jurisdiction/legal review.
