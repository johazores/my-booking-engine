...
Bookings or amendments created before the pricing-evidence migration may legitimately have no evidence row. SF does not reconstruct historical legal line items from today's mutable pricing configuration because labels, tax rules, applicability, and other descriptive/legal facts may have changed. The implemented Australian legal-document boundary fails closed when required historical pricing evidence is absent; older legal line items are not reconstructed from mutable current pricing rules.

This accepted-state pricing evidence is still **not itself a jurisdiction-specific legal invoice**. SF separately implements the narrow Australian hospitality legal-document lifecycle with immutable issuer/recipient/preparation evidence, server-derived fiscal numbering, immutable tax-invoice and supported adjustment-note issuance, deterministic PDF projection for the current supported text contract, tenant registers/accounting CSV, reconciliation, and retention boundaries. Mixed/partial/non-standard-GST semantics, generic correction/void/reissue, durable customer re-authentication/email delivery, universal Unicode-safe PDF rendering, production database validation, and jurisdiction/legal review remain open. See `docs/invoice-foundation.md` for the explicit legal boundary.
...
For invoice/tax work, the narrow Australian issuer/jurisdiction and legal-document lifecycle is already implemented over immutable accepted-state pricing evidence. The next coherent dependencies are the still-open Phase 12 boundaries: broader mixed/partial/non-standard-GST and correction/void/reissue semantics, durable customer re-authentication/email delivery/resend, universal Unicode-safe deterministic PDFs, production Node 24/PostgreSQL validation, and jurisdiction/legal review. The existing customer-safe payment receipt remains separate settlement evidence and never substitutes for legal-document authority.
