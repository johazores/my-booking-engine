...
- booking version evidence (`updatedAt`) is PostgreSQL-authored and monotonic on every update, so stale-review and commercial-amendment authority cannot be forged by choosing an arbitrary application or direct-SQL timestamp;
...
The booking identity guard complements the original commercial/ownership immutability trigger. Lifecycle fields such as `status` and `cancelledAt` retain their supported mutation paths, while the durable booking primary key and creation timestamp can no longer be rewritten by direct database updates.

`updatedAt` is concurrency/version authority rather than caller-owned business evidence. PostgreSQL overwrites every supplied booking update timestamp with `clock_timestamp()`, or one microsecond after the retained version when needed to preserve strict monotonicity. Reschedule, substitution, cancellation, and commercial-amendment compare-and-set checks can therefore trust the retained booking version without accepting an arbitrary client, application, or direct-SQL timestamp.
