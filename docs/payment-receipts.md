...
SF separately persists append-only `HospitalityBookingPricingEvidence` for newly accepted booking/commercial states. That evidence freezes the canonical nightly, tax/fee, and add-on breakdown together with exact aggregates, commercial scope, stay, selections, and pricing fingerprint. It is written through protected server booking/amendment transactions and is not customer/browser authority. Historical bookings created before the evidence migration can legitimately have no such row and are not automatically reconstructed from today's pricing configuration.

SF separately implements a narrow Australian hospitality legal-document lifecycle. Versioned `InvoiceIssuerProfile` and `HospitalityInvoicePreparation` evidence freeze issuer, recipient, pricing, and exact money; Australian readiness verifies the supported AU/AUD/ABN/standard-GST shape; `issueHospitalityAustralianTaxInvoice` performs serializable fiscal numbering and immutable issuance; supported adjustment notes are separately numbered; and verified documents can be projected through the current deterministic PDF, tenant register/accounting CSV, reconciliation, and retention boundaries. Tenant-entered registrations remain declarations rather than live registration verification.

That legal-document subsystem does **not** make this payment receipt a tax invoice. The payment receipt remains customer-safe settlement evidence and never borrows legal authority from an issued invoice or adjustment note. It does not invent or copy fiscal numbers, ABN/GST legal wording, tax-document status, adjustment semantics, or accounting authority. Those are exposed only through separately authorized and verified legal-document workflows. Durable customer re-authentication/email delivery, universal Unicode-safe PDF rendering, broader correction semantics, production database validation, and jurisdiction/legal review remain outside this receipt contract. See `docs/invoice-foundation.md` and `docs/australian-tax-invoice-contract.md` for the current legal-document boundary.
