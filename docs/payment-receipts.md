...
SF also has internal versioned `InvoiceIssuerProfile` evidence and deterministic `HospitalityInvoicePreparation` records that bind one exact issuer profile to matching accepted booking pricing evidence. Australia is the first explicit jurisdiction content-readiness contract: it verifies a narrow AU/AUD/ABN/GST evidence shape without issuing a regulated document. Tenant-entered registrations are declarations rather than live authority verification, invoice preparation/readiness is not issuance, and SF still has no fiscal numbering, immutable recipient/billing snapshot, issued invoice/credit-note lifecycle, legal-document rendering/PDF, delivery/history, retention, or accounting contract.

The presence of those immutable foundations and the Australian readiness contract does **not** make this payment receipt a tax invoice. The receipt therefore does not invent invoice numbers, ABN/GST legal assertions, tax-invoice wording, PDF tax invoices, or accounting synchronization. See `docs/invoice-foundation.md` and `docs/australian-tax-invoice-contract.md` for the production boundary and remaining dependencies.
