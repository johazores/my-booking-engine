...
SF also has internal versioned `InvoiceIssuerProfile` evidence and deterministic `HospitalityInvoicePreparation` records that bind one exact issuer profile to matching accepted booking pricing evidence. Those records are legal-document prerequisites only: tenant-entered registrations are not authority-verified jurisdiction support, invoice preparation is not issuance, and there is still no fiscal numbering, supported-jurisdiction tax contract, immutable billing identity, invoice/credit-note lifecycle, legal wording, PDF/rendering, delivery, retention, or accounting contract.

The presence of those immutable foundations does **not** make this payment receipt a jurisdiction-specific tax invoice. The receipt therefore does not invent invoice numbers, VAT/GST registration values, legal tax wording, PDF tax invoices, or accounting synchronization. See `docs/invoice-foundation.md` for the production boundary and remaining dependencies.
