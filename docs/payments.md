...
Stripe amendment settlement is also isolated from generic booking-payment writes. Dedicated internal Stripe services move/reconcile amendment-owned refunds and additional charges while preserving the prepared booking snapshot. Provider-known ambiguous amendment operations can now also converge through the signed webhook boundary described below. Customer-facing collection/authentication and expiry/hold compensation remain required before any amendment settlement action is user-facing.
...
There is intentionally no amendment-payment or amendment-apply API route yet. The internal manual and Stripe amendment executors, `commercialAmendmentId`, settlement decision, signed provider-known callback finalizer, and final apply service are infrastructure, not browser-controlled fields or signals that the full provider lifecycle is ready for users.
