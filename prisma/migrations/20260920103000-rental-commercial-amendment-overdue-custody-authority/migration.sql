-- Commercial-amendment fresh authority must also respect overdue open custody using
-- PostgreSQL time observed after the shared physical-unit advisory lock. This keeps
-- preparation, new adjustment evidence, and final apply from authorizing a unit that
-- another booking still physically retains after its effective committed end. The
-- amendment's own booking is deliberately excluded so a supported custody extension can
-- reconcile its own overdue booking. Compensation remains available because it reverses
-- already-retained money instead of creating fresh inventory authority.
...
BEGIN
    ...
    IF sf_rental_unit_has_overdue_custody(
        ...
        RAISE EXCEPTION 'rental commercial amendment cannot prepare a unit retained by another overdue open custody'
            USING ERRCODE = '23P01';
    END IF;
...
BEGIN
    ...
    IF sf_rental_unit_has_overdue_custody(
        ...
        RAISE EXCEPTION 'rental commercial amendment adjustment cannot use a unit retained by another overdue open custody'
            USING ERRCODE = '23P01';
    END IF;
...
BEGIN
    IF OLD."status" = 'PREPARED' AND NEW."status" = 'APPLIED' THEN
        ...
        IF sf_rental_unit_has_overdue_custody(
            ...
            RAISE EXCEPTION 'rental commercial amendment apply cannot use a unit retained by another overdue open custody'
                USING ERRCODE = '23P01';
        END IF;
    END IF;
