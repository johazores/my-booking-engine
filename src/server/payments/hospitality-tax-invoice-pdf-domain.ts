...
export class HospitalityTaxInvoicePdfValidationError extends Error {
  constructor(message: string) {
    super(message);
    this.name = 'HospitalityTaxInvoicePdfValidationError';
  }
}
...
export type HospitalityTaxInvoicePdfDocument = Readonly<{
  ...
  lines: readonly Readonly<{ description: string; quantity: number; amountMinor: string }>[];
...
function assertText(value: unknown, label: string, maxLength = PDF_MAX_TEXT_LENGTH) {
  if (typeof value !== 'string') throw new HospitalityTaxInvoicePdfValidationError(`${label} must be a string.`);
  const normalized = value.trim().replace(/\s+/g, ' ');
  if (!normalized || normalized.length > maxLength) {
    throw new HospitalityTaxInvoicePdfValidationError(`${label} must be between 1 and ${maxLength} characters.`);
  }
...
function nonNegativeMinor(value: unknown, label: string) {
  if (typeof value !== 'string' || !/^\d+$/.test(value)) {
    throw new HospitalityTaxInvoicePdfValidationError(`${label} must be a non-negative integer minor-unit string.`);
  }
  return BigInt(value);
}
...
function encodeWindows1252(value: string, label = 'PDF text') {
  ...
  for (const character of value) {
    ...
    throw new HospitalityTaxInvoicePdfValidationError(
...
function validateDocument(document: HospitalityTaxInvoicePdfDocument) {
  if (!document || typeof document !== 'object') throw new HospitalityTaxInvoicePdfValidationError('Tax invoice document is required.');
  if (document.documentTitle !== 'Tax invoice') throw new HospitalityTaxInvoicePdfValidationError('Only Australian tax invoices can be rendered.');
  const documentNumber = assertText(document.documentNumber, 'documentNumber', 64);
  if (!/^AU-TAX-[0-9]{8,}$/.test(documentNumber)) throw new HospitalityTaxInvoicePdfValidationError('documentNumber is invalid.');
  if (document.currency !== 'AUD') throw new HospitalityTaxInvoicePdfValidationError('Deterministic tax-invoice PDF currently supports AUD only.');
  if (!/^\d{11}$/.test(document.supplierAbn)) throw new HospitalityTaxInvoicePdfValidationError('supplierAbn is invalid.');
  if (document.buyerAbn !== null && !/^\d{11}$/.test(document.buyerAbn)) throw new HospitalityTaxInvoicePdfValidationError('buyerAbn is invalid.');
  if (!Array.isArray(document.lines) || document.lines.length === 0 || document.lines.length > PDF_MAX_LINES) {
    throw new HospitalityTaxInvoicePdfValidationError(`Tax invoice must contain between 1 and ${PDF_MAX_LINES} supply lines.`);
  }
  ...
  for (const line of document.lines) {
    assertText(line.description, 'supply description', 300);
    if (!Number.isSafeInteger(line.quantity) || line.quantity < 1 || line.quantity > 100_000) {
      throw new HospitalityTaxInvoicePdfValidationError('Supply quantity is invalid.');
    }
    nonNegativeMinor(line.amountMinor, 'supply amountMinor');
  }
  const subtotal = nonNegativeMinor(document.subtotalBeforeGstMinor, 'subtotalBeforeGstMinor');
  const gst = nonNegativeMinor(document.gstMinor, 'gstMinor');
  const total = nonNegativeMinor(document.totalMinor, 'totalMinor');
  if (subtotal + gst !== total) throw new HospitalityTaxInvoicePdfValidationError('Tax invoice totals do not reconcile.');
  assertText(document.taxableSaleStatement, 'taxableSaleStatement', 300);
  dateAu(document.issuedAt);
...
function pageHeader(documentNumber: string, page: number) {
  return [
    { text: 'Tax invoice', x: PDF_MARGIN, y: 790, size: PDF_TITLE_FONT_SIZE, bold: true },
    { text: documentNumber, x: 390, y: 794, size: 10, bold: true },
...
function createPageComposer(documentNumber: string) {
  ...
  return { add, addWrapped, ensure, finish };
}

function createContentPages(document: HospitalityTaxInvoicePdfDocument) {
  ...
  composer.add(`Issued ${dateAu(document.issuedAt)}`, { gapAfter: 10 });
  ...
  if (document.buyerAbn) composer.add(`ABN ${document.buyerAbn}`);
  ...
  composer.add('Supply', { bold: true });
  for (const line of document.lines) {
    const amount = formatAudMinor(line.amountMinor);
    const prefix = `${line.quantity} x `;
    const descriptionLines = wrapText(line.description, 58);
    composer.ensure(descriptionLines.length + 1);
    composer.addWrapped(`${prefix}${line.description}`, 68, { x: PDF_MARGIN + 8 });
    composer.add(`Amount excl. GST: ${amount}`, { x: PDF_MARGIN + 24, size: PDF_SMALL_FONT_SIZE, gapAfter: 2 });
  }

  composer.add('', { size: 1, gapAfter: 6 });
  composer.add(`Subtotal excl. GST: ${formatAudMinor(document.subtotalBeforeGstMinor)}`, { bold: true });
  composer.add(`GST: ${formatAudMinor(document.gstMinor)}`, { bold: true });
  composer.add(`Total incl. GST: ${formatAudMinor(document.totalMinor)}`, { bold: true, gapAfter: 6 });
  composer.addWrapped(document.taxableSaleStatement, 82, { size: PDF_SMALL_FONT_SIZE });
...
export function createHospitalityTaxInvoicePdf(document: HospitalityTaxInvoicePdfDocument) {
  ...
  for (let number = 1; number <= highestObjectNumber; number += 1) {
    ...
    if (!object) throw new HospitalityTaxInvoicePdfValidationError('Internal PDF object graph is incomplete.');
