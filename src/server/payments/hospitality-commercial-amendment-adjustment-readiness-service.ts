...
export class HospitalityCommercialAmendmentAdjustmentReadinessUnavailableError extends Error {
  constructor(message = 'Commercial-amendment adjustment readiness is not available.') {
    super(message);
    this.name = 'HospitalityCommercialAmendmentAdjustmentReadinessUnavailableError';
  }
}

export class HospitalityCommercialAmendmentAdjustmentReadinessPersistenceError extends Error {
  constructor(message: string) {
    super(message);
    this.name = 'HospitalityCommercialAmendmentAdjustmentReadinessPersistenceError';
  }
}
...
function normalizeSourceInvoiceNumber(value: string) {
  ...
  if (!AUSTRALIAN_TAX_INVOICE_NUMBER_PATTERN.test(normalized)) {
    throw new HospitalityCommercialAmendmentAdjustmentReadinessUnavailableError();
  }
  return normalized;
}
...
function validateSourceInvoice(row: {
  ...
  try {
    ...
    if (
      ...
      throw new HospitalityCommercialAmendmentAdjustmentReadinessPersistenceError(
    ...
    if (error instanceof HospitalityCommercialAmendmentAdjustmentReadinessPersistenceError) throw error;
    throw new HospitalityCommercialAmendmentAdjustmentReadinessPersistenceError(
...
}, amendmentId: string): AustralianCommercialAmendmentAdjustmentPrice {
  try {
...
export async function assessHospitalityCommercialAmendmentAdjustmentReadiness(input: {
  organizationId: string;
  actorUserId: string;
  bookingId: string;
  ...
  return db.$transaction(async (transaction) => {
    ...
    if (!sourceInvoice) throw new HospitalityCommercialAmendmentAdjustmentReadinessUnavailableError();
    ...
    if (!amendment) throw new HospitalityCommercialAmendmentAdjustmentReadinessUnavailableError();
