...
export async function verifyHospitalityCommercialAmendmentAdjustmentRowsInTransaction(input: {
  ...
  for (const rows of groups.values()) {
    const first = rows[0]!;
    const verified = await loadVerifiedHospitalityCommercialAmendmentAdjustmentChain({
    ...
    for (const row of rows) {
      if (!verifiedIds.has(row.id)) throw new Error('Commercial adjustment note is not present in its verified source chain.');
    }
  }
}
...
export async function verifyHospitalityCommercialAmendmentAdjustmentRows(input: {
  ...
  return db.$transaction(
    (transaction) => verifyHospitalityCommercialAmendmentAdjustmentRowsInTransaction({
      transaction,
      organizationId: input.organizationId,
      rows: input.rows,
    }),
