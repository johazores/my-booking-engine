...
async function verifiedWriteChain(input: {
  ...
  try {
    ...
    if (error instanceof HospitalityCommercialAmendmentAdjustmentChainLimitError) {
      throw new HospitalityCommercialAmendmentIncreasingAdjustmentNoteConflictError(error.message);
    }
    if (
      ...
      throw new HospitalityCommercialAmendmentIncreasingAdjustmentNotePersistenceError(error.message);
    }
    throw error;
  }
}

export async function issueHospitalityRepeatedCommercialAmendmentIncreasingAdjustmentNote(input: {
  organizationId: string;
  actorUserId: string;
  bookingId: string;
    ...
    try {
      return await db.$transaction(async (transaction) => {
        ...
        if (chain.priorAdjustmentNoteCount < 1 || !chain.head) {
          throw new HospitalityCommercialAmendmentIncreasingAdjustmentNoteConflictError(
            'Repeated increasing issuance requires a verified predecessor adjustment note.',
          );
        }
        ...
        const [paymentHistory, competingBaselineAmendmentCount] = await Promise.all([
          readHospitalityLegalPaymentEvidenceHistory({
            transaction,
            organizationId: input.organizationId,
            bookingId: input.bookingId,
        ...
        if (competingBaselineAmendmentCount !== 0) {
          throw new HospitalityCommercialAmendmentIncreasingAdjustmentNoteConflictError(
            'Multiple applied commercial amendments compete for the verified adjustment-note chain head.',
          );
        }
        ...
        if (!readiness.contentReady) {
          throw new HospitalityCommercialAmendmentIncreasingAdjustmentNoteConflictError(
            firstReadinessFailure(readiness),
          );
        }
        if (
          ...
          throw new HospitalityCommercialAmendmentIncreasingAdjustmentNotePersistenceError(
            'Increasing-adjustment readiness no longer matches the verified legal-document chain head.',
          );
        }
        ...
        if (
          ...
          throw new HospitalityCommercialAmendmentIncreasingAdjustmentNoteConflictError(
            'Commercial amendment chronology is incompatible with the verified chain head.',
          );
        }
        ...
        if (snapshot.schemaVersion !== 5) {
          throw new HospitalityCommercialAmendmentIncreasingAdjustmentNotePersistenceError(
            'Repeated increasing adjustment-note evidence must use schema version 5.',
          );
        }
        ...
        if (
          ...
          || reloadedChain.head.adjustmentType !== 'INCREASING'
          || reloadedChain.expectedSourceAdjustmentOrdinal !== sourceAdjustmentOrdinal + 1
        ) {
          throw new HospitalityCommercialAmendmentIncreasingAdjustmentNotePersistenceError(
            'Issued repeated increasing adjustment note did not become the verified legal-document chain head.',
          );
        }
