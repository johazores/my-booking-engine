...
test('hospitality taxes and fees enforce tenant scope, dependencies, and exact quote totals', async () => {
  ...
  try {
    ...
    assert.equal(quote.fees.amountMinor, '40000');
    assert.equal(quote.total.amountMinor, '488000');
    assert.equal(quote.charges.length, 2);
    ...
    const revalidated = await pricing.revalidateHospitalityBasePrice({ organizationId: organization.id, actorUserId: staff.id, request: { propertyId: property.id, roomTypeId: roomType.id, ratePlanId: ratePlan.id, arrivalDate: '2026-09-10', departureDate: '2026-09-12', quantity: '2' }, expectedFingerprint: quote.fingerprint });
    assert.equal(revalidated.changed, true);
    assert.equal(revalidated.latest.total.amountMinor, '440000');
