...
export async function setRentalUnitOperationalStatus(input: Readonly<{
  ...
  return db.$transaction(async (transaction) => {
    ...
    const current = await transaction.rentalUnitOperationalState.findFirst({
      where: {
        organizationId: input.organizationId,
        unitId: unit.id,
      },
    ...
    if (currentStatus === operational.status && currentReason === operational.reason) {
      return Object.freeze({
        state: current ?? {
          organizationId: input.organizationId,
          unitId: unit.id,
    ...
    let state;
    if (current) {
      const updated = await transaction.rentalUnitOperationalState.updateMany({
        where: {
          id: current.id,
          organizationId: input.organizationId,
          unitId: unit.id,
      ...
      state = await transaction.rentalUnitOperationalState.findFirst({
        where: {
          id: current.id,
          organizationId: input.organizationId,
          unitId: unit.id,
      ...
      state = await transaction.rentalUnitOperationalState.create({
        data: {
          organizationId: input.organizationId,
          unitId: unit.id,
          status: operational.status,
