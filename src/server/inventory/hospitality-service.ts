    ...
    return await db.$transaction(async (transaction) => {
      ...
      await transaction.auditEvent.create({
        data: {
          ...
          action: 'inventory.property.created',
          resourceType: 'hospitality-property',
          resourceId: created.id,
          afterData: { code: created.code, status: created.status },
        },
      });
      return created;
  ...
  try {
    return await db.$transaction(async (transaction) => {
      const property = await transaction.hospitalityProperty.findFirst({
        where: { id: roomType.propertyId, organizationId: input.organizationId, status: 'ACTIVE' },
        select: { id: true },
      });
      ...
      await transaction.auditEvent.create({
        data: {
          organizationId: input.organizationId,
          actorUserId: input.actorUserId,
          action: 'inventory.room-type.created',
          resourceType: 'hospitality-room-type',
          resourceId: created.id,
    ...
    return await db.$transaction(async (transaction) => {
      ...
      await transaction.auditEvent.create({
        data: {
          ...
          resourceType: 'hospitality-room',
          resourceId: created.id,
          afterData: { propertyId: created.propertyId, roomTypeId: created.roomTypeId, code: created.code, status: created.status },
        },
      });
      return created;
    }, { isolationLevel: 'Serializable' });
...
async function archiveInventoryRecord(input: { organizationId: string; actorUserId: string; resourceId: string; confirmation: string; kind: 'property' | 'room-type' | 'room' }) {
  ...
  return db.$transaction(async (transaction) => {
    ...
    if (input.kind === 'property') {
      const current = await transaction.hospitalityProperty.findFirst({
        where: { id: input.resourceId, organizationId: input.organizationId, status: 'ACTIVE' },
      ...
      const updated = await transaction.hospitalityProperty.update({ where: { id: current.id, organizationId: input.organizationId }, data: { status: 'ARCHIVED', archivedAt } });
      await transaction.auditEvent.create({
    ...
    if (input.kind === 'room-type') {
      const current = await transaction.hospitalityRoomType.findFirst({
        where: { id: input.resourceId, organizationId: input.organizationId, status: 'ACTIVE' },
        select: { id: true, status: true },
      });
      if (!current) throw new HospitalityInventoryUnavailableError();
      ...
      const updated = await transaction.hospitalityRoomType.update({ where: { id: current.id, organizationId: input.organizationId }, data: { status: 'ARCHIVED', archivedAt } });
      await transaction.auditEvent.create({
    ...
    const updated = await transaction.hospitalityRoom.update({ where: { id: current.id, organizationId: input.organizationId }, data: { status: 'ARCHIVED', archivedAt } });
    await transaction.auditEvent.create({
      data: { organizationId: input.organizationId, actorUserId: input.actorUserId, action: 'inventory.room.archived', resourceType: 'hospitality-room', resourceId: current.id, beforeData: { status: current.status }, afterData: { status: 'ARCHIVED', archivedAt: archivedAt.toISOString() } },
    });
    return updated;
  }, { isolationLevel: 'Serializable' });
}
