  ...
  return <form className="sf-booking-modification-form" onSubmit={review}>
    ...
    {options && options.assignments.length > 0 ? <>
      ...
      {preview ? <section className="sf-booking-modification-preview" aria-labelledby="commercial-price-preview-title" aria-live="polite">
        <div><p className="sf-eyebrow">Current price review</p><h3 id="commercial-price-preview-title">{preview.direction === 'NONE' ? 'No payment adjustment required' : preview.direction === 'ADDITIONAL_CHARGE' ? 'Additional payment would be required' : 'Refund would be required'}</h3></div>
        <dl className="sf-booking-modification-preview__totals"><div><dt>Current total</dt><dd>{preview.currentTotalDisplay}</dd></div><div><dt>Proposed total</dt><dd>{preview.proposedTotalDisplay}</dd></div><div><dt>Price adjustment</dt><dd>{preview.adjustmentDisplay}</dd></div></dl>
      ...
      {!preview ? <p className="sf-muted"><small>Price-changing edits stay review-only until SF has a durable amendment and payment-adjustment workflow. Zero-delta changes remain executable after review.</small></p> : null}
    </> : null}
  </form>;
}
